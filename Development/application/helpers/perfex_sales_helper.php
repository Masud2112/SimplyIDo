...
function get_invoice_total_left_to_pay($id, $invoice_total)
{
    ...
    foreach ($payments as $payment) {
        $invoice_total -= ($payment['amount']-$payment['gratuity_val']);
    }
...
function update_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    ...
    if (isset($payments[0])) {
        if ($payments[0]['amount'] == $invoice->total) {
            ...
            //if ($total == ($invoice->total - $invoice->transaction_charge) || $total > ($invoice->total - $invoice->transaction_charge)) {
    ...
    if ($CI->db->affected_rows() > 0) {
        if($status==2){
            ...
            $CI->db->where('templateid', $proposalId);
            $CI->db->update('tblproposaltemplates', $data);
        }
        if ($prevent_logging == true) {
            return $status;
        }
        logActivity('Invoice Status Updated [Invoice Number: ' . format_invoice_number($invoice->id) . ', From: ' . format_invoice_status($original_status, '') . ' To: ' . format_invoice_status($status, '') . ']', null);

        $additional_activity = serialize(array(
            '<original_status>' . format_invoice_status($original_status) . '</original_status>',
            '<new_status>' . format_invoice_status($status) . '</new_status>'
        ));
...
{
    $CI =& get_instance();
    $CI->db->select('*')->from('tblinvoices')->where('id', $id);
    ...
    $proposal_number = "";
    $proposal = "";
    if ($invoice->proposal_number > 0) {
        $proposal_number = str_pad($invoice->proposal_number, 2, '0', STR_PAD_LEFT);
        if ($invoice->leadid > 0 && !is_null($invoice->leaddate)) {
            $invoice->date = $invoice->leaddate;
        } elseif ($invoice->project_id > 0 && !is_null($invoice->projectdate)) {
            $invoice->date = $invoice->projectdate;
        }
        $proposal = "P";
        if ($format == 1) {
            // Number based
            return $prefix .$proposal. str_pad($proposal_number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        } elseif ($format == 2) {
            return $prefix . $proposal . date('Y', strtotime($invoice->date)). str_pad($proposal_number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        } elseif ($format == 3) {
            return $prefix . $proposal . date('Ymd', strtotime($invoice->date)). str_pad($proposal_number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        } elseif ($format == 4) {
            $inv_no = $invoice->eventno . $invoice->eventinvoiceno;
            return $prefix . $proposal . date('Ymd', strtotime($invoice->leaddate)) . '/' . str_pad($inv_no, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT). str_pad($proposal_number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        }
    } else {
        if ($format == 1) {
            // Number based
            ...
            return $prefix . $proposal . date('Y', strtotime($invoice->date)) . $proposal_number . '/' . str_pad($invoice->number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        } elseif ($format == 3) {
            return $prefix . $proposal . date('Ymd', strtotime($invoice->date)) . $proposal_number . '/' . str_pad($invoice->number, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        } elseif ($format == 4) {
            $inv_no = $invoice->eventno . $invoice->eventinvoiceno;
            return $prefix . $proposal . date('Ymd', strtotime($invoice->leaddate)) . '/' . str_pad($inv_no, get_option('number_padding_prefixes'), '0', STR_PAD_LEFT);
        }
    }
...
function get_proposaltemplate_item_taxes($itemid)
{
    $CI =& get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'proposaltemplate');
    $taxes = $CI->db->get('tblitemstax')->result_array();
                    ...
                    if (!is_numeric($mode['id']) && !empty($mode['id'])) {
                        foreach ($invoice->allowed_payment_modes as $allowed_mode) {
                            if ($allowed_mode == $mode['id']) {
                                ...
                                $currencies = explode(',', get_brand_option('paymentmethod_' . $mode['id'] . '_currencies'));
                                foreach ($currencies as $currency) {
                                    $currency = trim($currency);
...
function pdf_logo_url()
{
    ...
    if ($custom_pdf_logo_image_url != '') {
        ...
        if (get_option('company_logo') != '') {
            ...
            $cimg = base_url('uploads/brands/round_' . $company_logo);
        } else {
            $cimg = '';
        }
    }
    ...
    $logo_url = '';
    if ($company_logo != '') {
        $path = get_upload_path_by_type('brands'). '/' . $company_logo;
        if(file_exists($path)){
            $path = get_upload_path_by_type('brands'). '/round_' . $company_logo;
            $src = base_url('uploads/brands/' . $company_logo);
            if(file_exists($path)){
                $src = base_url('uploads/brands/round_' . $company_logo);
            }
        }
        $logo_url = '<a href="' . $logo_href . '" class="logo img-responsive"><img width="100" src="' . $src . '" class="img-responsive" alt="' . $company_name . '" ></a>';
    } else if ($company_name != '') {
        $logo_url = '<a href="' . $logo_href . '" class="logo">' . $company_name . '</a>';
    } else {
        $logo_url = '';
    }
...
function proposal_pdf($proposal, $tag = '')
{
    ...
    $proposal_url = site_url('viewproposal/' . $proposal->templateid);
    $number = format_proposal_number($proposal);
    ...
    if ($proposal->proposal_total != 0) {
        $total = format_money($proposal->proposal_total, $CI->currencies_model->get($proposal->currency)->symbol);
...
function load_invoices_total_template()
{
    ...
    if ($CI->input->post('project_id') && $CI->input->post('project_id') != "") {
        $_data['project_id'] = $CI->input->post('project_id');
    }

    if ($CI->input->post('eventid')) {
        $_data['eventid'] = $CI->input->post('eventid');
    }
...
if (!function_exists('format_customer_info')) {
    ...
    function format_customer_info($data, $for, $type, $companyLink = false)
    {
        ...
        if ($for == 'statement') {
            ...
            $companyName = $data->client->firstname . ' ' . $data->client->lastname;//$data->client->company; 
        }

        if ($for == 'invoice' || $for == 'estimate' || $for == 'payment') {
            if (isset($data->client->show_primary_contact)) {
        ...
        if ($companyLink) {
            $companyName = '<a href="javascript:void(0)"><b>' . $companyName . '</b></a>';
        } else if ($companyName != '') {
...
if (!function_exists('format_organization_info')) {
    ...
    function format_organization_info()
    {
        $brandid = get_user_session();
        if ($brandid > 0) {
            $format = get_brand_option('company_info_format');
            $vat = get_brand_option('company_vat');

            $format = _info_format_replace('company_name', '<b style="color:black" class="company-name-formatted">' . get_brand_option('invoice_company_name') . '</b>', $format);
            $format = _info_format_replace('address', get_brand_option('invoice_company_address'), $format);
            $format = _info_format_replace('city', get_brand_option('invoice_company_city'), $format);
            $format = _info_format_replace('state', get_brand_option('company_state'), $format);

            $format = _info_format_replace('zip_code', get_brand_option('invoice_company_postal_code'), $format);
            $format = _info_format_replace('country_code', get_brand_option('invoice_company_country_code'), $format);
            $format = _info_format_replace('phone', get_brand_option('invoice_company_phonenumber'), $format);
            $format = _info_format_replace('vat_number', $vat, $format);
...
function format_proposal_number($proposal="")
{
    ...
    return $prefix . str_pad($nextproposalnumber, $pad_length, '0', STR_PAD_LEFT);
}