...
function render_tags($tags)
{
    ...
    if (count($tags) > 0) {
        ...
        foreach ($tags as $tag) {
            $tag_id = 0;
            $tag_color = '';
            $CI->db->select('id,color')->where('name', $tag);
...
function render_venue_tags($tags)
{
    ...
    if (count($tags) > 0) {
        ...
        foreach ($tags as $tag) {
            $tag_id = 0;
            $tag_color = '';
            $CI->db->select('name,color')->where('id', $tag);
            $tag_row = $CI->db->get('tbltags')->row();
            if ($tag_row) {
                $tag_id = $tag_row->name;
                $tag_color = $tag_row->color;
            }
            $tags_html .= '<span class="label label-tag tag-id-' . $tag_id . '"><span class="tag">' . $tag_id . '</span><span class="hide">' . ($i != $len - 1 ? ', ' : '') . '</span></span>';
            $i++;
        }
        $tags_html .= '</div>';
    }
...
function render_yes_no_option($option_value, $label, $tooltip = '')
{
    ...
    ?>>
        ...
        <div class="radio radio-primary radio-inline">
            ...
            echo $option_value;
            ?>]" value="1" <?php
            if (get_brand_option($option_value) == '1') {
                echo 'checked';
            }
            ?>>
        ...
        <div class="radio radio-primary radio-inline">
            ...
            echo $option_value;
            ?>]" value="0" <?php
            if (get_brand_option($option_value) == '0') {
                echo 'checked';
            }
            ?>>
...
{
    //Added by vaidehi on 03/03/2018 to get brand option
    if (get_user_session() > 0) {
        $date_format = get_brand_option('dateformat');
    } else {
    ...
    <script>
        $(function () {
            <?php if(get_user_session() > 0) { ?>
            var time_format = '<?php echo get_brand_option('time_format'); ?>';
            <?php } else { ?>
            ...
            <?php if(get_user_session() > 0) { ?>
            var calendar_first_day = '<?php echo get_brand_option('calendar_first_day'); ?>';
            <?php } else { ?>
            ...
            if (time_format == 24) {
                date_time_picker_options.format = date_format + ' G:i';
            } else {
...
<?php } ?>
    <link href="<?php echo base_url('assets/plugins/datetimepicker/jquery.datetimepicker.min.css'); ?>"
          rel="stylesheet">
...
function render_datatable($headings = array(), $class = '', $additional_classes = array(''), $table_attributes = array())
{
    ...
    $table = '<div class="' . $IEfix . '"><table' . $_table_attributes . 'class="dt-table-loading table table-striped sdtheme table-' . $class . '' . $_additional_classes . '">';
    $table .= '<thead>';
    $table .= '<tr>';
...
{
    $company_logo = get_brand_option('company_logo', $brandid);
    $company_name = get_brand_option('companyname', $brandid);
...
function staff_profile_image($id = false, $classes = array('staff-profile-image'), $type = 'round', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('profile_image,firstname,lastname,profilecolor');
    ...
    if ($result && $result->profile_image != null && !empty($result->profile_image)) {

        $profileImagePath = 'uploads/staff_profile_images/' . $id . '/' . $type . '_' . $result->profile_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<div class="profImgDiv"><img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->firstname . ' ' . $result->lastname . '" /></div>';
        } else {
    ...
    } elseif ($result->profilecolor !== '' || empty($result->profile_image)) {
        $profile_image = '<span ' . $_attributes . ' class="no-img ' . implode(' ', $classes) . '" style="background-color:' . $result->profilecolor . '">' . strtoupper(substr($result->firstname, 0, 1)) . strtoupper(substr($result->lastname, 0, 1)) . '</span>';
    } else {
...
function staff_pdf_profile_image($id = false, $classes = array('signer_pic'), $type = 'round', $img_attrs = array())
{
...
function addressbook_profile_image($id = false, $classes = array('addressbook-profile-image'), $type = 'round', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('profile_image,firstname,lastname,profilecolor');
    $CI->db->where('addressbookid', $id);
    $result = $CI->db->get('tbladdressbook')->row();
    $_attributes = '';
    foreach ($img_attrs as $key => $val) {
        $_attributes .= $key . '=' . '"' . $val . '" ';
    }

    $blankImageFormatted = '<div class="profImgDiv"><img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" /></div>';
    ...
    if ($result && $result->profile_image !== null && !empty($result->profile_image)) {
        ...
        $profileImagePath = 'uploads/addressbook_profile_images/' . $id . '/' . $type . '_' . $result->profile_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->firstname . ' ' . $result->lastname . '" />';
        } else {
        ...
        $profile_image = '<span ' . $_attributes . ' class="no-img ' . implode(' ', $classes) . '" style="background-color:' . $result->profilecolor . '">' . strtoupper(substr($result->firstname, 0, 1)) . strtoupper(substr(preg_replace("/[^a-zA-Z]/", "", $result->lastname), 0, 1)) . '</span>';
    } else {
        $profile_image = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" alt="' . $result->firstname . ' ' . $result->lastname . '" />';
    }
    return $profile_image;
...
function addressbook_pdf_profile_image($id = false, $classes = array('signer_pic'), $type = 'round', $img_attrs = array())
{
    $url = base_url('assets/images/user.png');
...
function contact_profile_image($id = false, $classes = array('contact-profile-image'), $type = 'small', $img_attrs = array())
{
    ...
    if ($result && $result->profile_image !== null) {
        ...
        $profileImagePath = 'uploads/contact_profile_images/' . $id . '/' . $type . '_' . $result->profile_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->firstname . ' ' . $result->lastname . '" />';
        } else {
            return $blankImageFormatted;
...
function venue_logo_image($id = false, $classes = array('venue-logo-image'), $type = 'round', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('venuelogo, venuename');
    $CI->db->where('venueid', $id);
    $result = $CI->db->get('tblvenue')->row();
    ...
    $blankImageFormatted = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" />';
    if (!empty($result)) {
        $blankImageFormatted = '<span ' . $_attributes . ' class="no-img ' . implode(' ', $classes) . '" style="background-color:">' . strtoupper(substr($result->venuename, 0, 2)) . '</span>';
    }
    ...
    if ($result && $result->venuelogo !== null && !empty($result->venuelogo)) {

        $profileImagePath = 'uploads/venue_logo_images/' . $id . '/' . $type . '_' . $result->venuelogo;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->venuename . '" />';
        } else {
            return $blankImageFormatted;
        }
    } else {
        $profile_image = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" alt="' . $result->venuename . '" />';
        $profile_image = $blankImageFormatted;
    }
    return $profile_image;
}
...
function venue_cover_image($id = false, $classes = array('venue-cover-image'), $type = 'thumb', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('venuecoverimage, venuename');
    $CI->db->where('venueid', $id);
    $result = $CI->db->get('tblvenue')->row();
    ...
    if ($result && $result->venuecoverimage !== null && !empty($result->venuecoverimage)) {

        $profileImagePath = 'uploads/venue_cover_images/' . $id . '/' . $type . '_' . $result->venuecoverimage;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->venuename . '" />';
        } else {
            return $blankImageFormatted;
...
function proposaltemplate_banner($id = false, $classes = array('proposalbanner-image'), $type = '', $img_attrs = array())
{
    $url = base_url('assets/images/default_banner.png');

    $CI =& get_instance();
    $CI->db->select('banner,name');
    $CI->db->where('templateid', $id);
    $result = $CI->db->get('tblproposaltemplates')->row();
    ...
    if ($result && $result->banner !== null && !empty($result->banner)) {
        $bannerPath = 'uploads/proposals_images/banner/' . $id . '/' . $result->banner;
        if ($type !== "") {
            $bannerPath = 'uploads/proposals_images/banner/' . $id . '/' . $type . '_' . $result->banner;
        }
        if (file_exists($bannerPath)) {
            $banner = '<img ' . $_attributes . ' src="' . base_url($bannerPath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
        } else {
            return $blankImageFormatted;
        }
    } else {
        $banner = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
    }
    $banner = "<div class='proposal_list_image'>" . $banner . "</div>";
    return $banner;
}
...
function lead_profile_image($id = false, $classes = array('lead-profile-image'), $type = 'round', $img_attrs = array())
{
    ...
    if ($result && $result->profile_image !== null) {
        ...
        $profileImagePath = 'uploads/lead_profile_images/' . $id . '/' . $type . '_' . $result->profile_image;
        if (file_exists($profileImagePath)) {
...
{
    $url = base_url('assets/images/default_banner.jpg');

    $CI =& get_instance();
    $CI->db->select('project_profile_image,name');
    $CI->db->where('id', $id);
    ...
    if ($result && $result->project_profile_image !== null) {

        $profileImagePath = 'uploads/project_profile_images/' . $id . '/' . $type . '_' . $result->project_profile_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
        } else {
...
function project_cover_image($id = false, $classes = array('project-cover-image'), $type = '', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('projectcoverimage,name');
    $CI->db->where('id', $id);
    $result = $CI->db->get('tblprojects')->row();
    ...
    if ($result && $result->projectcoverimage !== null) {

        $profileImagePath = 'uploads/project_cover_images/' . $id . '/' . $type . '_' . $result->projectcoverimage;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
        } else {
            return $blankImageFormatted;
...
function lead_files($id = false, $classes = array('lead-profile-image'), $type = 'small', $img_attrs = array())
{
    $url = base_url('assets/images/user-placeholder.jpg');
...
{
    $url = base_url('assets/images/no-package.png');

    $CI =& get_instance();
    $CI->db->select('profile_image,description');
    $CI->db->where('id', $id);
    $result = $CI->db->get('tblitems')->row();
    ...
    if ($result && $result->profile_image !== null) {
        ...
        $profile_image = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" alt="' . $result->description . '" />';
    }
    return $profile_image;
}
...
function app_happy_text($text)
{
    ...
    $regex = do_action('app_happy_regex', 'congratulations|congrats|happy|feel happy|awesome');
    $re = '/' . $regex . '/i';
...
function icon_url($url = '', $type = '', $class = '', $attributes = array())
{
    if ($type == "eye") {
        $title = "View";
    ...
    return '<li><a href="' . $_url . '" class=" ' . $class . ' btn-icon" ' . $_attributes . '><i class="fa fa-' . $type . '"></i><span>' . $title . '</span></a></li>';
}
    ...
    ?> table-tickets">
        ...
        <tr>
            ...
            if ($bulk_action == true) {
                ...
                <th>
                    ...
                    <div class="checkbox mass_select_all_wrap"><input type="checkbox" id="mass_select_all"
                                                                      data-to-table="tickets"><label></label></div>
                </th>
                <?php
            }
            ?>
            ...
            <th><?php
                ...
                ?></th>
            <th<?php if (get_option('services') == 0) {
                echo ' class="not_visible"';
            } ?>><?php
...
{
    $CI = &get_instance();
    $statuses = $CI->tasks_model->get_status();
...
function get_lead_status_by_id($id)
{
    $CI = &get_instance();
    ...
    function get_table_items_and_taxes($items, $type, $admin_preview = false)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($admin_preview == true) {
                ...
                $td_first_sortable = ' class="item_no"';
            }
            ...
            if (count($item_taxes) > 0) {
                foreach ($item_taxes as $tax) {
                    ...
                    if ($tax_not_calc == true) {
                        ...
                        array_push($result['taxes'][$tax['taxname']]['total'], (($item['qty'] * ($item['rate'] + $item['markupdiscount'])) / 100 * $tax['taxrate']));
...
/* Added by Purvi for get brand logo */
function get_client_brand_logo($uri = '', $href_class = '', $brandid)
{
    $company_logo = get_client_brand_option('company_logo', $brandid);
    ...
    if ($company_logo != '') {
        echo '<a href="' . $logo_href . '" class="' . $href_class . ' logo img-responsive"><img src="' . base_url('uploads/brands/' . $company_logo) . '" class="img-responsive" alt="' . $company_name . '"></a>';
    } else if ($company_name != '') {
        echo '<a href="' . $logo_href . '" class="' . $href_class . ' logo">' . $company_name . '</a>';
    } else {
        echo '';
...
function group_image($id = false, $classes = array('group-image'), $type = 'small', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('group_image,name');
    $CI->db->where('id', $id);
    $result = $CI->db->get('tblitems_groups')->row();
    ...
    if ($result && $result->group_image !== null) {

        $profileImagePath = 'uploads/product_services_package_images/' . $id . '/' . $type . '_' . $result->group_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
        } else {
            return $blankImageFormatted;
...
function gallery_proposal_image($id = false, $pid, $classes = array('gallery-image'), $type = 'round', $img_attrs = array())
{
    ...
    $CI =& get_instance();
    $CI->db->select('title,name,type');
    $CI->db->where('id', $id);
    $result = $CI->db->get('tblproposal_gallery_files')->row();
    ...
    if ($result && $result->name !== null) {
        if ($result->type == 'gallery') {
            ...
            $profileImagePath = 'uploads/proposals_images/gallery/' . $pid . '/' . $type . '_' . $result->name;
        } else {
            $profileImagePath = 'uploads/proposals_images/files/' . $pid . '/' . $type . '_' . $result->name;
        }
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->name . '" />';
        } else {
        ...
        $profile_image = '<img src="' . $url . '" ' . $_attributes . ' class="' . implode(' ', $classes) . '" alt="' . $result->title . '" />';
    }
    return $profile_image;
}

function subscription_payment_gateway_head($title = 'Payment for Subscription'){
ob_start(); ?>
<!DOCTYPE html>
<html lang="en">
<head>
