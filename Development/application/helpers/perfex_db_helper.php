...
function get_project_col_by_id($id, $col = "name")
{
    $CI =& get_instance();
    $CI->db->select($col);
    $CI->db->where('id', $id);
...
function get_lead_col_by_id($id, $col = "name")
{
    $CI =& get_instance();
    $CI->db->select($col);
    $CI->db->where('id', $id);
    $project = $CI->db->get('tblleads')->row();
    if ($project) {
        return $project->$col;
    }
...
function get_brand_option($name)
{
    ...
    return $CI->perfex_base->get_brand_option($name);
}

function get_client_brand_option($name, $brandid)
{
    $CI =& get_instance();
    if (!class_exists('perfex_base')) {
        $CI->load->library('perfex_base');
    ...
    return $CI->perfex_base->get_client_brand_option($name, $brandid);
}
...
function update_brand_option($name, $value, $autoload = null)
{
    ...
    $brandoption = $CI->db->get('tblbrandsettings')->row();
    $data = array(
        'value' => $value
    );
...
function get_staff_randum_password($userid = '')
{
    $_userid = get_staff_user_id();
    if ($userid !== '') {
        $_userid = $userid;
...
function is_staff_active($userid = '')
{
    $_userid = get_staff_user_id();
    if ($userid !== '') {
        $_userid = $userid;
...
function get_staff_phone($userid = '')
{
    ...
    $CI =& get_instance();
    $CI->db->where('staffid', $_userid);
    $staff = $CI->db->select('phonenumber')->from('tblstaff')->get()->row();
    if ($staff) {
...
function get_addressbook_full_name($userid = '')
{
    $CI =& get_instance();
    $CI->db->where('addressbookid', $userid);
    $addressbook = $CI->db->select('firstname,lastname')->from('tbladdressbook')->get()->row();
    if ($addressbook) {
...
function get_addressbook_phone($userid = '')
{
    $CI =& get_instance();
    $CI->db->where('addressbookid', $userid);
    $CI->db->where('type', 'primary');
    $addressbook = $CI->db->select('phone,ext')->from('tbladdressbookphone')->get()->row();

    if ($addressbook) {
        if (!empty($addressbook->ext)) {
            return $addressbook->phone . "  x" . $addressbook->ext;
        }
...
function get_addressbook_tags($contactid, $user_id)
{
    $brandid = get_user_session();
    $CI =& get_instance();
    $CI->db->where('tbladdressbooktags.addressbookid', $contactid);
    $CI->db->where('tbltags.id=tbladdressbooktags.tagid');
    $CI->db->where('tbladdressbooktags.brandid=tbltags.brandid');
    $CI->db->where('tbltags.deleted=0');
    $CI->db->join('tbladdressbooktags', 'tbladdressbooktags.brandid =' . $brandid);
    $tags = $CI->db->select('GROUP_CONCAT(tbltags.name) as tags')->from('tbltags')->get()->row();
    if ($tags) {
        return $tags->tags;
    } else {
        return '';
...
function get_addressbook_email($userid = '')
{
    $CI =& get_instance();
    $CI->db->where('addressbookid', $userid);
    $CI->db->where('type', 'primary');
    $addressbook = $CI->db->select('email')->from('tbladdressbookemail')->get()->row();
    if ($addressbook) {
...
function get_addressbook_title($userid = '')
{

    $CI =& get_instance();
    $CI->db->where('addressbookid', $userid);
    $addressbook = $CI->db->select('companytitle')->from('tbladdressbook')->get()->row();
    if ($addressbook) {
        return $addressbook->companytitle;
    } else {
        return '';
    }
}
...
function logCronActivity($description)
{
    ...
    $CI->db->insert('tblcronactivitylog', $log);
}
...
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = array(), $where = array(), $additionalSelect = array(), $sGroupBy = '', $limit = "", $start = "")
{
    ...
    } elseif (is_numeric($start) && $limit != -1) {
    ...
    if ($CI->input->post('order')) {
        ...
        foreach ($CI->input->post('order') as $key => $val) {
            ...
            if ($sTable == 'tbladdressbook') {
                $columnName = $aColumns[intval($__post['order'][$key]['column'])];

                if ($columnName == 17) {
                    $columnName = "email";
                } elseif ($columnName == 18) {
                    $columnName = "phone";
                } elseif ($columnName == 27) {
                    $columnName = "tags";
                }
                // }elseif($sTable == 'tblstafftasks'){
                //     $columnName = $aColumns[intval($__post['order'][$key]['column'])];
                //     if($columnName == 27){
                //         $columnName = "tags";
                //     }
     ...
     * Output
     */
    $__post['draw'] = isset($__post['draw']) ? $__post['draw'] : 0;
    $output = array(
...
function get_tags_in_addressbook($tags)
{
    $CI =& get_instance();
    $tag_names = array();
    foreach ($tags as $tag) {
...
function tasks_rel_name_select_query()
{
    return '(CASE rel_type
        ...
        WHEN "invoice" THEN (SELECT tblinvoices.id FROM tblinvoices WHERE tblinvoices.id = tblstafftasks.rel_id)
...
function get_choice_by_name($name, $itemid)
{
    $CI =& get_instance();
    $result = $CI->db->query('SELECT GROUP_CONCAT(choice_name ORDER BY `order` ASC) AS choice_name FROM tblitems_choices WHERE deleted = 0 AND option_id = (SELECT id FROM tblitems_options WHERE deleted =0 AND option_name = "' . $name . '" AND itemid = ' . $itemid . ' ORDER BY `id` DESC LIMIT 0,1) GROUP BY option_id');
    $raw_data = $result->row();
    return $raw_data;
}

function get_choice_rate_by_name($name, $itemid)
{
    $CI =& get_instance();
    $result = $CI->db->query('SELECT GROUP_CONCAT(choice_rate ORDER BY `order` ASC) AS choice_rate FROM tblitems_choices WHERE deleted = 0 AND option_id = (SELECT id FROM tblitems_options WHERE deleted =0 AND option_name = "' . $name . '" AND itemid = ' . $itemid . ' ORDER BY `id` DESC LIMIT 0,1) GROUP BY option_id');
    $raw_data = $result->row();
    return $raw_data;
}
...
*Added by Masud
*Date:02-05-2018
*To get Product and services by category ID
*/

function get_product_services_by_cat_id($cat_id)
{
    $session_data = get_session_data();
    $CI =& get_instance();
    $CI->db->select('id,description');
    $CI->db->from('tblitems');
    $CI->db->where('line_item_sub_category', $cat_id);
    $CI->db->where('brandid', $session_data['brand_id']);
    $CI->db->where('deleted', 0);
    $raw_data = $CI->db->get()->result();
    //$result = $CI->db->query('SELECT * FROM tblitems WHERE deleted = 0 AND line_item_sub_category = '.$cat_id);
    return $raw_data;
}
...
function get_meeting_assignee($meeting_id)
{
    $session_data = get_session_data();
    $CI =& get_instance();
    $CI->db->select('*');
    ...
    foreach ($assignees as $assignee) {
        ...
        if (!empty($member)) {
            $team_member[] = $member->firstname . " " . $member->lastname;
        }
        if (!empty($contact)) {
            $client[] = $contact->firstname . " " . $contact->lastname;
        }
    }
    $raw_data['member'] = $team_member;
    $raw_data['client'] = $client;
    return $raw_data;
}

function get_member_by_id($staffid)
{
    $session_data = get_session_data();
    $CI =& get_instance();
    $CI->db->select('*');
...
function get_contact_by_id($contact_id)
{
    ...
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tbladdressbook');
    $CI->db->where('addressbookid', $contact_id);
    $contact = $CI->db->get()->row();
    return $contact;
}

function get_tax_rate_by_id($id)
{
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tbltaxes');
...
function get_choice_rate_by_id($id)
{
    $CI =& get_instance();
    $CI->db->select('choice_rate');
    $CI->db->from('tblitems_choices');
    $CI->db->where('id', $id);
    $rate = $CI->db->get()->row()->choice_rate;
    return $rate;
}
...
function get_leads_by_status($statusid)
{
    $CI =& get_instance();
    $CI->db->where('tblleads.status', $statusid);
    $CI->db->where('tblleads.deleted = ', 0);
    $CI->db->where('tblleads.converted = ', 0);
    $CI->db->where('tblleads.brandid = ', get_user_session());
    if (!is_admin()) {
        $CI->db->join('tblstaffleadassignee', 'tblstaffleadassignee.leadid = tblleads.id', 'left');
        $CI->db->join('tblstaff', 'tblstaff.staffid = tblstaffleadassignee.assigned', 'left');
        $CI->db->where('(tblleads.addedfrom = ' . get_staff_user_id() . ' OR tblstaffleadassignee.assigned=' . get_staff_user_id() . ' OR tblleads.is_public = 1) GROUP BY tblleads.id');
    }
    $total = $CI->db->get('tblleads')->result();
    return count($total);
}

function get_projects_by_status($statusid)
{
    $CI =& get_instance();
    $CI->db->where('tblprojects.status', $statusid);
    $CI->db->where('tblprojects.deleted = ', 0);
    $CI->db->where('tblprojects.parent = ', 0);
    $CI->db->where('tblprojects.brandid = ', get_user_session());
    if (!is_admin()) {
        $CI->db->join('tblstaffprojectassignee', 'tblstaffprojectassignee.projectid = tblprojects.id', 'left');
        $CI->db->join('tblstaff', 'tblstaff.staffid = tblstaffprojectassignee.assigned', 'left');
        $CI->db->where('(tblprojects.addedfrom = ' . get_staff_user_id() . ' OR tblstaffprojectassignee.assigned=' . get_staff_user_id() . ') GROUP BY tblprojects.id');
    }
    $total = $CI->db->get('tblprojects')->result();
    return count($total);
}

function is_lead_converted($leadid)
{
    $CI =& get_instance();
    $CI->db->where('id', $leadid);
    $CI->db->where('converted', 1);
    $total = $CI->db->get('tblleads')->result();
    if (count($total) > 0) {
        return true;
    }
...
function get_lead_to_project_link($leadid)
{
    $CI =& get_instance();
    $CI->db->select('id');
    $CI->db->where('convert_type_id', $leadid);
...
{
    $CI =& get_instance();
    /*$CI->db->order_by('parent_category', 'asc');*/
    $CI->db->select('tbllineitem_subcategory.*, (SELECT name FROM tbllineitem_category WHERE id = `tbllineitem_subcategory`.`parent_id` ORDER BY `name`) as parent_category');
    $CI->db->where('brandid', get_user_session());
    $CI->db->where('deleted', '0');
    $CI->db->where('parent_id', $id);

    /*if ($id != '') {
        $CI->db->where('deleted', $id);
        return $CI->db->get('tbllineitem_subcategory')->row();
    }*/
...
function get_lead_assignee($leadid)
{
    ...
    $CI->db->where('tblstaffleadassignee.leadid', $leadid);
...
function get_project_assignee($id)
{
    ...
    $CI->db->where('tblstaffprojectassignee.projectid', $id);
