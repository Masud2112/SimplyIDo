...
function get_available_date_formats()
{
    ...
    // );
    $date_formats = array(
        'm/d/Y|%m/%d/%Y' => '12/31/' . date('Y'),
        'd/m/Y|%d/%m/%Y' => '31/12/' . date('Y'),
        'Y-m-d|%Y-%m-%d' => date('Y') . '-12-31'
    );
...
/*
    Created by Purvi on 10-31-2017
*/
function after_time($time_ago)
{
    $time_ago = strtotime($time_ago);
    $cur_time = time();
    ...
    // Seconds
    if ($seconds <= 60) {
        return _l('next_time_just_now');
    } //Minutes
    elseif ($minutes <= 60) {
        if ($minutes == 1) {
            return _l('next_time_minute');
        } else {
            return _l('next_time_minutes', $minutes);
        }
    } //Hours
    elseif ($hours <= 24) {
        if ($hours == 1) {
            return _l('next_time_hour');
        } else {
            return _l('next_time_hours', $hours);
        }
    } //Days
    elseif ($days <= 7) {
        return _l('next_time_days', $days);
    } //Weeks
    elseif ($weeks <= 4.3) {
        if ($weeks == 1) {
            return _l('next_time_week');
        } else {
            return _l('next_time_weeks', $weeks);
        }
    } //Months
    elseif ($months <= 12) {
        if ($months == 1) {
            return _l('next_time_month');
        } else {
            return _l('next_time_months', $months);
        }
    } //Years
    else {
        if ($years == 1) {
            return _l('next_time_year');
        } else {
            return _l('next_time_years', $years);
        }
    }
}
...
function get_timezones_list()
{
     ...
     * Modified By : Vaidehi
     * Dt : 11/14/2017
     * to get time zone in our desired format
     */
    $allowed = ['America/Anchorage', 'America/Chicago', 'America/Los_Angeles', 'America/New_York'];

    $replacement = array(1 => 'Alaska Time', 35 => 'Central Time', 81 => 'Pacific Time', 100 => 'Eastern Time');
    ...
    return array_combine(array_filter($timezones['AMERICA'], function ($value) use ($allowed) {
...
function app_select_plugin_js($locale = 'en')
{
    ...
    if ($locale != 'en') {
        if (file_exists(FCPATH . 'assets/plugins/bootstrap-select/js/i18n/defaults-' . $locale . '.min.js?v=' . get_app_version())) {
            echo "<script src='" . base_url('assets/plugins/bootstrap-select/js/i18n/defaults-' . $locale . '.min.js?v=' . get_app_version()) . "'></script>" . PHP_EOL;
        } elseif (file_exists(FCPATH . 'assets/plugins/bootstrap-select/js/i18n/defaults-' . $locale . '_' . strtoupper($locale) . '.min.js?v=' . get_app_version())) {
            echo "<script src='" . base_url('assets/plugins/bootstrap-select/js/i18n/defaults-' . $locale . '_' . strtoupper($locale) . '.min.js?v=' . get_app_version()) . "'></script>" . PHP_EOL;
        }
    }
}
...
{
    echo "<script src='" . base_url('assets/plugins/jquery-validation/jquery.validate.js?v=' . get_app_version()) . "'></script>" . PHP_EOL;
    if ($locale != 'en') {
        if (file_exists(FCPATH . 'assets/plugins/jquery-validation/localization/messages_' . $locale . '.min.js?v=' . get_app_version())) {
            echo "<script src='" . base_url('assets/plugins/jquery-validation/localization/messages_' . $locale . '.min.js?v=' . get_app_version()) . "'></script>" . PHP_EOL;
        } elseif (file_exists(FCPATH . 'assets/plugins/jquery-validation/localization/messages_' . $locale . '_' . strtoupper($locale) . '.min.js?v=' . get_app_version())) {
            echo "<script src='" . base_url('assets/plugins/jquery-validation/localization/messages_' . $locale . '_' . strtoupper($locale) . '.min.js?v=' . get_app_version()) . "'></script>" . PHP_EOL;
        }
    }
}
...
//            'help' => _l('help_tasks_permissions')
        ...
        /*Added by Vaidehi on 10/12/2017 */
        'brands' => array(
            'view' => true,
            'view_own' => false,
            'edit' => true,
        ...
        /*Added by Vaidehi on 12/18/2017 */
 ...
 * Added By : Vaidehi
 * Dt: 10/03/2017
 * for package permission
 */
/**
 * All permissions available in the app with conditions
 * @return array
 */
function get_package_permission_conditions()
{
    return do_action('staff_permissions_conditions', array(
        'contracts' => array(
            'access' => true
...
function render_admin_js_variables()
{
    ...
    $lang = array(
        ...
        'drop_file_here_to_upload' => _l('drop_file_here_to_upload'),
        'drop_image_here_to_upload' => _l('drop_image_here_to_upload'),
        ...
        if ($imapAutoImportingTicketsConfigured || $leadsEmailIntegrationConfigured) {
            ...
            <div class="col-md-12">
                <div class="alert alert-danger">
                    ...
                    You can enable it via php.ini or contact your hosting provider to enable this extension.
                </div>
            </div>
        <?php }
    }
    ...
    if ($header && !empty($header) && get_option('show_cloudflare_notice') == '1' && is_admin()) {
        ...
        <div class="col-md-12">
            <div class="alert alert-warning font-medium">
                ...
                <h4><strong>Cloudflare usage detected</strong></h4>
                <hr/>
                <ul>
                    <li>When using Cloudflare with the application <strong>you must disable ROCKET LOADER</strong>
                        feature from Cloudflare options in order everything to work properly. <br/><strong>
                            <small>NOTE: The script can't check if Rocket Loader is enabled/disabled in your Cloudflare
                                account. If Rocket Loader is already disabled you can ignore this warning.
                            </small>
                        </strong></li>
                    <li>
                        <br/>
                        <ul>
                            ...
                            <li>&nbsp;&nbsp;&nbsp;&nbsp;Login to your Cloudflare account and click on the
                                <strong>Speed</strong> tab from the top dashboard, search for Rocket Loader and <strong>set
                                    to Off</strong>.
                            </li>
                            <br/>
                            <li><strong>&nbsp;&nbsp;- Disable Rocket Loader with page rule for application installation
                                    url</strong></li>
                            <li>
                                &nbsp;&nbsp;&nbsp;&nbsp;If you do not want to turn off Rocket Loader for the whole
                                domain you can add <a
                                        href="https://support.cloudflare.com/hc/en-us/articles/200168306-Is-there-a-tutorial-for-Page-Rules-"
                                        target="_blank">page rule</a> that will disable the Rocket Loader only for the
                                application, follow the steps below in order to achieve this.
                                <br/><br/>
                                <p class="no-margin">&nbsp;&nbsp;- Login to your Cloudflare account and click on the
                                    <strong>Page Rules</strong> tab from the top dashboard</p>
                                <p class="no-margin">&nbsp;&nbsp;- Click on <strong>Create Page Rule</strong></p>
                                <p class="no-margin">&nbsp;&nbsp;- In the url field add the following url:
                                    <strong><?php echo rtrim(site_url(), '/') . '/'; ?>*</strong></p>
                                <p class="no-margin">&nbsp;&nbsp;- Click <strong>Add Setting</strong> and search for
                                    <strong>Rocket Loader</strong></p>
                                <p class="no-margin">&nbsp;&nbsp;- After you select Rocket Loader <strong>set value to
                                        Off</strong></p>
                ...
                <br/><br/><a href="<?php echo admin_url('misc/dismiss_cloudflare_notice'); ?>" class="alert-link">Got
                    it! Don't show this message again</a>&nbsp;&nbsp;&nbsp;&nbsp;<a
                        href="<?php echo admin_url('misc/dismiss_cloudflare_notice'); ?>" class="alert-link">Rocket
                    loader is already disabled</a>
            </div>
        </div>
        <?php
...
function tasks_summary_data($rel_id = null, $rel_type = null)
{
    ...
    foreach ($statuses as $status) {
        ...
        $tasks_where .= ' AND brandid = ' . $brandid;
...
function show_pdf_unable_to_get_image_size_error()
{
    ...
    <div style="font-size:17px;">
        ...
        <p>Very often this is happening <b>when you are using custom PDF logo url in Setup -> Settings -> PDF</b>, first
            make sure that the url you added in Setup->Settings->PDF for the custom pdf logo is valid and the image
            exists if the problem still exists you will need to use a <b>direct path</b> to the image to include in the
            PDF documents. Follow the steps mentioned below:</p>
        <p><strong>Method 1 (easy)</strong></p>
        <ul>
            <li>Upload the logo image in the installation directory eq. <?php echo FCPATH; ?>mylogo.jpg</li>
            <li><a href="<?php echo admin_url('settings?group=pdf'); ?>" target="_blank">Navigate to Setup -> Settings
                    -> PDF</a> -> Custom PDF Company Logo URL and only add the filename like: <b>mylogo.jpg</b>, now
                Custom PDF Company Logo URL should be only filename not full URL.
            </li>
        ...
        <ul>
            <li>Consult with your hosting provider to confirm that the server is able to use PHP's <a
                        href="http://php.net/manual/en/function.file-get-contents.php"
                        target="_blank">file_get_contents</a> or <a
                        href="http://php.net/manual/en/curl.examples-basic.php" target="_blank">cUrl</a> to download the
                file.
            </li>
            <li>Try to re-generate PDF document again.</li>
        </ul>
...
function get_meeting_reminders()
{
    /*$reminders = array(
        '0' => 'on time',
        '60' => '1m before',
...
/* Added by Purvi on 10-24-2017 */
function get_email_phone_type()
{
    $email_phone_type = array(
        'personal' => 'Personal',
        'work' => 'Work',
        'other' => 'Other'
    );

    return $email_phone_type;
}
...
function get_duedate_criteria()
{
    $duedate_criteria = array(
        'beforeproject' => 'Before project date',
        'afterproject' => 'After project date',
        'afterinvoice' => 'After invoice has been sent'
    );

    return $duedate_criteria;
}

function get_duedate_duration()
{
    $duedate_duration = array(
        'days' => 'Day(s)',
        'weeks' => 'Week(s)',
        'months' => 'Month(s)'
    );

    return $duedate_duration;
}
...
function _parse_agreement_template_merge_fields($agreement, $merge_fields)
{
    foreach ($merge_fields as $key => $val) {
        if (stripos($agreement, $key) !== false) {
            $agreement = str_ireplace($key, $val, $agreement);
        } /*else {
            $agreement = str_ireplace($key, '', $agreement);
        }*/
