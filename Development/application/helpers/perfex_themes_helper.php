...
* Added By : Vaidehi
* Dt : 10/24/2017
* get active theme for brand
*/
/**
 * Get active brand theme
 * @return mixed
 */
function active_brand_theme()
{
    $CI =& get_instance();

    $theme = get_brand_option('clients_default_theme');
    if ($theme == '') {
        show_error('Default theme is not set');
    }
    if (!is_dir(APPPATH . 'views/themes/' . $theme)) {
...
function do_theme_required_head($params = array()){
    ...
    <script>
        ...
        var tinymce_lang = '<?php echo get_tinymce_language(get_locale_key("en")); ?>',
