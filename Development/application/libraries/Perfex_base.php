...
class Perfex_Base
{
    ...
    public function __construct()
    {
        ...
        */
        if(isset($session_data['brand_id']) && $session_data['brand_id'] != "") {
            $brand_options = $this->_instance->db->select('name, value')
            ->where('isvisible ', 1)
            ->where('brandid ', $session_data['brand_id'])
            ->get('tblbrandsettings')->result_array();

            // Loop the options and store them in a array to prevent fetching again and again from database
            foreach ($brand_options as $brand_option) {
                $this->brand_options[$option['name']] = $brand_option['value'];
            }
        }
    ...
    */
     ...
     * @return string
     */
    public function get_brand_option($name,$brandid = "")
    {
        $session_data = get_session_data();
        ...
        if($path != ""){
            ...
            if(isset($fullpath[2]) && $fullpath[2] != ""){
                $mid_data = $fullpath[2];
                $bid = explode('brand_id=',$mid_data);
                $bid = end($bid);
            }
        }
        if (!isset($this->brand_options[$name])) {
            if($brandid == "" && isset($session_data['brand_id'])){
            ...
            $this->_instance->db->select('value');
            $this->_instance->db->where('name = ', $name);
            $this->_instance->db->where('isvisible = ', 1);
            $this->_instance->db->where('brandid = ', (isset($brandid)) ? $brandid : $bid);
            $row = $this->_instance->db->get('tblbrandsettings')->row();
            if ($row) {
                return $row->value;
            } else {
                return '';
            ...
            return $this->brand_options[$name];
        }
    ...
    /*
        Added by Purvi on 11-27-2017 for get brand details
    */
    public function get_client_brand_option($name, $brandid)
    {
        if ($name == 'number_padding_invoice_and_estimate') {
            $name = 'number_padding_prefixes';
        }
        ...
        if (!isset($this->brand_options[$name])) {
            ...
            $this->_instance->db->where('name = ', $name);
            $this->_instance->db->where('isvisible = ', 1);
            $this->_instance->db->where('brandid = ', $brandid);
            $row = $this->_instance->db->get('tblbrandsettings')->row();
            if ($row) {
                return $row->value;
            } else {
