...
<div class="<?php echo strtolower($title) ?>_header psl_section_head ">
    <div class="row psl_section_head_blk">
        ...
        <div class="col-md-6 ">
            ...
            <?php if (strtolower($title) != 'send proposal') { ?>
    ...
    if ((isset($_GET['pid']) && $_GET['pid'] > 0) || (isset($_GET['pid']) && $_GET['pid'] > 0) || isset($token)) {
        if (isset($rel_content)) {
            ...
            $esdate = _time($rel_content->eventstartdatetime);

            $eenddate = _time($rel_content->eventenddatetime);
                        ...
                        <div class="pl_circle">
                            <?php if (isset($lead)) {
                                ...
                                if (file_exists($profileImagePath)) {
                                    $pl_logo = lead_profile_image($rel_content->id, array());
                                } else {
                                ...
                                if (isset($profileImagePath) && file_exists($profileImagePath)) {
                                    $pl_logo = project_profile_image($rel_content->id, array());
                                } else {
                                    $pl_init = substr($ename, 0, 1);
                                }
                            }
                    ...
                    <?php if (strtolower($title) != 'send proposal') { ?>
                        <div class="pl_inner inline-block">
                            <div class="proposal">
                                ...
                                <strong>
                                    <?php if (strtolower($title) == "invoice") {
                                        echo($invoice_prefix ? $invoice_prefix : "");
                                    } else { ?>
                                        #<?php echo strtoupper(substr($title, 0, 1)) ?>
                                        -
                                    <?php } ?>
                                </strong>
                                <?php echo isset($proposal) ? $proposalversion : 00001 ?>
                            </div>
                            ...
                            <?php } ?>
                            <?php if (strtolower($title) == "invoice") {
                                echo format_invoice_status($nextinvoice->status);
                            } ?>
                        </div>
                    <?php } else { ?>
                        <div class="proposal_status">
