...
<div class="quote_items">
    ...
    foreach ($selected_items as $selected_item) {
        if (strtolower($selected_item['type']) == 'package') {
            ...
            $item = $this->invoice_items_model->get_item($selected_item['id']);
        }
        ?>
        <div class="item">
            <h5 class="itemTitle"><strong><?php echo ucfirst($item->description )?></strong></h5>
            ...
            <?php if (strtolower($selected_item['type']) == 'package') {
                ...
                <ul>
                    <?php foreach ($package_items as $key => $package_item) {
                        $lineintem = $this->invoice_items_model->get_item($key);
                        if(!empty($lineintem)){ ?>
