...
$eventstartdatetime = date('l, F j, Y', strtotime($invoice->project_data->eventstartdatetime));
$eventstarttime = date('h:i A', strtotime($invoice->project_data->eventstartdatetime));
...
$invoice_banner = '<div class="bannerImg"><img src="' . base_url() . 'assets/images/default_banner.jpg" style="max-width: 100%"/></div>';
$y = $pdf->getY();

$invoiceeventInfo = '<div>';
$invoiceeventInfo .= project_profile_image($invoice->project_data->id) . '<br />';
$invoiceeventInfo .= $invoice->project_data->name . '<br />';
$invoiceeventInfo .= $eventstartdatetime . '<br />';
$invoiceeventInfo .= $eventstarttime . '<br />';
$invoiceeventInfo .= '</div>';

$invoiceeventInfo2 = '<div>';
$invoiceeventInfo2 .= '<b style="color:#4e4e4e;"># ' . $invoice_number . '</b>' . '<br />';
$invoiceeventInfo2 .= _l('invoice_data_date') . ' ' . _d($invoice->date) . '</b>' . '<br />';
$invoiceeventInfo2 .= _l('invoice_data_duedate') . ' ' . _d($invoice->duedate) . '</b>' . '<br />';
$invoiceeventInfo2 .= '</div>';
...
$invoice_info .= '</div>';
$pdf->writeHTMLCell(0, '', '', $y, $invoice_banner, 0, 1, false, true, ($swap == '1' ? 'J' : 'R'), true);
$y = $pdf->getY();

$pdf->MultiCell(($dimensions['wk'] / 2) - $dimensions['lm'], 0, $invoiceeventInfo, 0, 'J', 0, 0, '', '', true, 0, true, true, 0);
// write the second column
$pdf->MultiCell(($dimensions['wk'] / 2) - $dimensions['rm'], 0, $invoiceeventInfo2, 0, 'R', 0, 1, '', '', true, 0, true, false, 0);
$pdf->ln(6);
$y = $pdf->getY();
...
//$pdf->writeHTMLCell(($swap == '1' ? ($dimensions['wk']) - ($dimensions['lm'] * 2) : ($dimensions['wk'] / 3) - $dimensions['lm']), '', '', $y, $invoice_info, 0, 0, false, true, ($swap == '1' ? 'R' : 'J'), true);
//$pdf->writeHTMLCell(($dimensions['wk'] / 3) - $dimensions['rm'], '', '', $y, $invoice_info, 0, 0, false, true, ($swap == '1' ? 'R' : 'J'), true);

// Bill to
//$client_details = '<b>' . _l('invoice_bill_to') . '</b><br />';
$client_details = '';
$client_details .= '<div style="color:#424242;">';
$client_details .= format_customer_info($invoice, 'invoice', 'billing') . '<br />';
$client_details .= $invoice->client->phone->phone . '<br />';
$client_details .= $invoice->client->email->email . '<br />';
$client_details .= '</div>';

$invoicestatus = "";
if (get_option('show_status_on_pdf_ei') == 1) {
    $invoicestatus .= '<br /><span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_invoice_status($status, '', false) . '</span>';
}

if ($status != 2 && $status != 5 && get_option('show_pay_link_to_invoice_pdf') == 1) {
    $invoicestatus .= ' - <a style="color:#84c529;text-decoration:none;text-transform:uppercase;" href="' . site_url('viewinvoice/' . $invoice->id . '/' . $invoice->hash) . '"><1b>' . _l('view_invoice_pdf_link_pay') . '</1b></a>';
}
...
<table width="100%" bgcolor="#fff">
    <tr>
        ...
        <td width="75%" align="left">' . $invoice_info . '</td></tr></table></td>
        <td width="33.33%" align="left"><table width="100%" bgcolor="#fff">
        <tr><td width="25%" align="left">TO : </td>
        <td width="75%" align="left">' . $client_details . '</td></tr></table></td>
        <td width="33.33%" align="right">' . $invoicestatus . '</td>
    </tr>
    </table>';
/*if (get_option('show_status_on_pdf_ei') == 1) {
    $invoicestatus .= '<br /><span style="color:rgb(' . invoice_status_color_pdf($status) . ');text-transform:uppercase;">' . format_invoice_status($status, '', false) . '</span>';
...
//$pdf->writeHTMLCell(($dimensions['wk'] / 3) - $dimensions['rm'], '', '', ($swap == '1' ? $y : ''), $client_details, 0, 1, false, true, 'J', true);

//$pdf->writeHTMLCell(($dimensions['wk'] / 3) - $dimensions['rm'], '', '', ($swap == '1' ? $y : ''), $invoicestatus, 0, 1, false, true, 'R', true);

$pdf->writeHTML($invoiceRow, true, false, false, false, '');
...
if (isset($invoice->proposal)) {
    ...
    <td align="right" width="15%">' . format_money($proposal->otherval, $invoice->symbol) . '</td>
</tr>';
    }
}
...
foreach ($invoice->payments as $payment) {
    $gratuity_percent = $gratuity_percent + $payment['gratuity_percent'];
    $gratuity_val = $gratuity_val + $payment['gratuity_val'];
}
if ($gratuity_percent > 0 && $gratuity_val > 0) {
    $tbltotal .= '<tr>
                                <td align="right" width="85%"><strong>' . _l("Gratuity") . '(' . $gratuity_percent . '%)</strong></td><td align="right" width="15%" class="total">' . format_money($gratuity_val, $invoice->symbol) . '</td></tr>';
    $invoice->total = $invoice->total + $gratuity_val;
}
$tbltotal .= '
...
if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1) {
    ...
    $tblhtml = '<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="0">
        ...
        <th width="20%;" style="' . $border . '">' . _l('invoice_payments_table_number_heading') . '</th>
        <th width="20%;" style="' . $border . '">' . _l('invoice_payments_table_mode_heading') . '</th>
        <th width="20%;" style="' . $border . '">' . _l('invoice_payments_table_date_heading') . '</th>';
    $tblhtml .= '<th width="20%;" style="' . $border . '">' . _l('Gratuity') . '</th>';
    $tblhtml .= '<th width="20%;" style="' . $border . '">' . _l('invoice_payments_table_amount_heading') . '</th>
    </tr>';
    $tblhtml .= '<tbody>';
    foreach ($invoice->payments as $payment) {
