...
<tr height="30" style="color:#fff;" bgcolor="#3A4656">
    <th width="' . $width . '%;">' . _l('payment_table_invoice_number') . '</th>
    <th width="' . $width . '%;">' . _l('payment_table_invoice_date') . '</th>
    <th width="' . $width . '%;">' . _l('payment_table_invoice_amount_total') . '</th>';
if ($payment->gratuity_val > 0) {
    $tblhtml .= '<th width="' . $width . '%;">' . _l('Gratuity') . '</th>';
}
$tblhtml .= '<th width="' . $width . '%;">' . _l('payment_table_payment_amount_total') . '</th>
</tr>';
$tblhtml .= '<tbody>';
$tblhtml .= '<tr>';
...
if ($payment->gratuity_val > 0) {
    $tblhtml .= '<td>' . format_money($payment->gratuity_val, $payment->invoice_data->symbol) . '</td>';
}
