...
<ul class="kan-ban-col" data-col-status-id="<?php //echo $status['statusid']; ?>"
    ...
    <li class="kan-ban-col-wrapper-venues">
        <div class="panel_s">
            <div class="kan-ban-content-wrapper" id="status_<?php //echo $status['statusid']; ?>">
                <div class="kan-ban-content">
                    <ul class="venues-status"
                        data-venue-status-id="<?php //echo $status['statusid']; ?>">
                        <?php
                        $total_venues = count($totalvenues);
                        $count = 1;
                        foreach ($venues as $venue) {
                            $this->load->view('admin/venues/_kan_ban_card', array('venue' => $venue, 'count' => $count));
                            $count++;
                        }
                        ?>
                        ...
                        <?php } ?>
                        <li class="text-center not-sortable mtop30 kanban-empty<?php if ($total_venues > 0) {
                            echo ' hide';
                        } ?>">
                            <h4 class="text-muted">
                                <i class="fa fa-circle-o-notch" aria-hidden="true"></i><br/><br/>
                                <?php echo _l('no_venues_found'); ?>
                            </h4>
                        </li>
                    </ul>
                </div>
...
<div class="col-sm-6 data-footer">
    ...
    <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
        ...
        of <?php echo $total_venues ?> entries
    </div>
</div>
<?php
$total_pages = $total_venues / $limit;
