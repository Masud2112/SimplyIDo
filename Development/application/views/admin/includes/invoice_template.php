<div class="panel_s<?php if(!isset($invoice) || (isset($invoice) && count($invoices_to_merge) == 0 && (isset($invoice) && !isset($invoice_from_project) && count($expenses_to_bill) == 0))){echo ' hide';} ?> hidden" id="invoice_top_info">
      ...
      <div class="row">
         <div class="col-md-6">                    
            ...
            <?php
               ...
               }else if ($format == 4 && isset($lid) && $lid != "") {
                 if(isset($invoice)){
                   $__number = $invoice->number;
                   $prefix = $invoice->prefix;
               ...
               if ($format == 4 && isset($lid) && $lid != "" && !isset($invoice)) {
            ...
            <?php $value = (isset($invoice) ? _dt($invoice->date, false) : _dt(date('Y-m-d'),false)); ?>
    ...
    <div class="row">
        ...
        <div class="form-group mbot25 items-wrapper">
          <select name="item_group_select" class="selectpicker no-margin<?php if($ajaxItems == true){echo ' ajax-search';} ?>" data-width="100%" id="item_group_select" data-none-selected-text="<?php echo _l('add_item_group'); ?>" data-live-search="true">
            ...
            <?php foreach($items as $group_id=>$_items){  ?>
              <option value="<?php echo $group_id; ?>"><?php echo $_items[0]['name']; ?></option>
            <?php } ?>                  
          </select>
        </div>
      </div>
      ...
      <div class="table-responsive s_table">
         <table class="table invoice-items-table items table-main-invoice-edit no-mtop">
            ...
            <tbody>
               <tr class="main">
                  <td class="item-img" style="padding: 10px 0"></td>
                  <td>
                 ...
                 <!--  <td>
                  ...
                  <td>
                     <?php
                        ...
                        ?>
                     <button type="button" onclick="add_item_to_table('undefined','undefined',<?php echo $new_item; ?>); return false;" class="btn pull-right btn-info"><i class="fa fa-plus"></i></button>
                  </td>
               </tr>
               <?php if (isset($invoice) || isset($add_items)) {
                  ...
                  foreach ($add_items as $item) {
                    ...
                    if($item['profile_image'] == "" || $item['profile_image'] == null){
                        $profileImagePath = base_url().'assets/images/no-package.png';
                    }else{
                        $profileImagePath = base_url().'uploads/line_item_images/'.$item['itemid'].'/small_'.$item['profile_image'];
                    }
                    $table_row .= '<td><img src="'.$profileImagePath.'" class="item-profile-image-small" alt="'.$item['description'].'"></td>';
