<div class="kan-ban-content-wrapper-forms" id="formsCard">
    <div class="kan-ban-content">
        <ul class="leadcaptureformss row">
            <?php
            $total_forms = count($forms);
            $count = 1;
            foreach ($forms as $form) {
                $this->load->view('admin/leadcaptureforms/_kan_ban_card', array('form' => $form, 'count' => $count));
                $count++;
            }
            ?>
            ...
            <?php } ?>
            <li class="text-center not-sortable mtop30 kanban-empty<?php if ($totalforms > 0) {
                echo ' hide';
            } ?>">
                <h4 class="text-muted">
...
<div class="col-sm-6 data-footer">
    ...
    <?php
    $end=$page*$limit;
    if($limit > count($forms) || $limit==-1){
        $end = count($totalforms);
    }
    ?>
    <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">Showing <?php echo $start= ($page-1)*$limit+1; ?> to <?php echo $end ?>
        of <?php echo count($totalforms); ?> entries
    </div>
</div>
<?php
$total_pages = count($totalforms)/$limit;
...
<script type="text/javascript">
    jQuery('#kan-ban .page_itmes_no').change(function () {
        leadcaptureforms_kanban('limitchanged');
    });
