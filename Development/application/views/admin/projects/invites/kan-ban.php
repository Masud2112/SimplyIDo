...
/*$invites = $this->invites_model->do_invite_kanban_query($status['statusid'], $this->input->get('search'), 1, array('sort_by' => $this->input->get('sort_by'), 'sort' => $this->input->get('sort')));*/
?>
<ul class="kan-ban-col" data-col-status-id="<?php //echo $status['statusid']; ?>"
    data-total-pages="<?php // echo $total_pages; ?>">
    <li class="kan-ban-col-wrapper-invites">
        <div class="panel_s">
            ...
            <div class="kan-ban-content-wrapper-invites" id="status_<?php //echo $status['statusid']; ?>">
                <div class="kan-ban-content">
                    <ul class="invites-status disFlex"
                        data-invite-status-id="<?php //echo $status['statusid']; ?>">
                        <?php
                        $total_invites = count($totalinvites);
                        /*if(isset($search) && !empty($search)){
                            $total_invites = count($invites);
                        }*/
                        $count = 1;
                        foreach ($invitees as $invite) {
                            $this->load->view('admin/projects/invites/_kan_ban_card', array('invite' => $invite,'count' => $count));
                            $count++;
                        }
                        ?>
                        ...
                        <?php } ?>
                        <li class="text-center not-sortable mtop30 kanban-empty<?php if ($total_invites > 0) {
                            echo ' hide';
                        } ?>">
                            <h4 class="text-muted">
                                <i class="fa fa-circle-o-notch" aria-hidden="true"></i><br/><br/>
                                <?php echo _l('no_invites_found'); ?>
                            </h4>
                        </li>
                    </ul>
                </div>
...
<div class="col-sm-6 data-footer">
    ...
    <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">Showing <?php echo $start= ($page-1)*$limit+1; ?> to <?php echo $end ?>
        of <?php echo $total_invites ?> entries
    </div>
</div>
<?php
$total_pages = $total_invites/$limit;
