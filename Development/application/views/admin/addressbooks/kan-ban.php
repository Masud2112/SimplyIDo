...
<ul class="kan-ban-col" data-col-status-id="<?php //echo $status['statusid']; ?>"
    ...
    <li class="kan-ban-col-wrapper-contacts">
        <div class="panel_s">
            <div class="kan-ban-content-wrapper" id="status_<?php //echo $status['statusid']; ?>">
                <div class="kan-ban-content">
                    <ul class="contacts-status"
                        data-contact-status-id="<?php //echo $status['statusid']; ?>">
                        <?php
                        $total_contacts = count($totalcontacts);
                        $count = 1;
                        foreach ($contacts as $contact) {
                            $this->load->view('admin/addressbooks/_kan_ban_card', array('contact' => $contact, 'count' => $count));
                            $count++;
                        }
                        ?>
                        ...
                        <?php } ?>
                        <li class="text-center not-sortable mtop30 kanban-empty<?php if ($total_contacts > 0) {
                            echo ' hide';
                        } ?>">
                            <h4 class="text-muted">
                                <i class="fa fa-circle-o-notch" aria-hidden="true"></i><br/><br/>
                                <?php echo _l('no_contacts_found'); ?>
                            </h4>
                        </li>
                    </ul>
                </div>
...
<div class="col-sm-6 data-footer">
    ...
    <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
        ...
        of <?php echo $total_contacts ?> entries
    </div>
</div>
<?php
