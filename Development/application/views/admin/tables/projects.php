...
/*if ($user_type == 1) {
    $aColumns = array(
        ...
        'tblprojects.name',
        '(SELECT CONCAT(tblvenue.venueaddress, \', \', tblvenue.venuecity, \', \', tblvenue.venuestate) FROM tblvenue WHERE tblvenue.venueid =  tblprojects.venueid) as venueaddress',
        '(SELECT venuename FROM tblvenue WHERE tblvenue.venueid =  tblprojects.venueid) as project_venue',
...
$join = array(
    ...
    'LEFT JOIN tblprojectcontact ON tblprojectcontact.projectid = tblprojects.id AND tblprojectcontact.active=1',
...
if (!$is_admin) {
    array_push($where, 'AND (tblstaffprojectassignee.assigned =' . get_staff_user_id() . ' OR addedfrom = ' . get_staff_user_id() . ' OR tblprojectcontact.contactid = ' . get_staff_user_id() . ')');
}
...
foreach ($rResult as $aRow) {
    ...
    $venue_logo="";
    if($aRow['venueid'] > 0 ){
        $venue = get_vanue_data($aRow['venueid']);
        $venue_logo = "<div class='venue'><div class='profImgDiv'>" . venue_logo_image($aRow['venueid'],array('img img-responsive staff-profile-image-small')) . "</div>
    ...
    $projectAssignees = get_project_assignee($aRow['id']);
    ...
    $eventdate .= "<div class='text-center'><small>" . _time($aRow['tblprojects.eventstartdatetime']) . "</small></div>";
    $row[] = $eventdate;
    ...
    $project_name .= '<div class="divInline_blk"><div class="project-pimg">' . project_profile_image($aRow['id'], array("project-profile-image-small")) . '</div><div class="project-det"><div class="project-name"><a href="' . admin_url('projects/dashboard/' . $aRow['id']) . '">' . $name . '</a></div>';
    $project_name .= "<div class='projecttypename'><i class='fa fa-book'></i>" . $aRow['eventtypename'] . "</div></div></div>";
    $row[] = $project_name;

    $row[] = (!empty($aRow['project_venue']) ? $venue_logo : '');
    //$row[] = '';

    $statuscontent = '<div class="divInline_blk"><span class="projectstatuscolor" style="background-color:' . $aRow['color'] . ';"></span>';
