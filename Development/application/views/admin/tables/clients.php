...
$join = array('INNER JOIN tblstaff ON tblstaff.staffid=tblclients.primary_user_id',
'INNER Join tblpackages ON tblpackages.packageid=tblclients.packageid');

array_push($where, 'AND (tblclients.is_deleted=0)');
...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, array(
    ...
    'tblclients.userid as userid',
    'tblclients.packageid as packageid'
));
...
foreach ($rResult as $aRow) {
    ...
    // Primary contact
    //$row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['name'] . '</a>' : '');
    //Added on 10/03 By Avni
    $row[] = ($aRow['staffid'] ? $aRow['name'] : '');
    ...
    $row[] = ($aRow['phonenumber'] ? '' . $aRow['phonenumber'] . '' : '');

     $row[] = ($aRow['package'] ? '<a target="_blank" href="packages/package/' . $aRow['packageid'] . '">' . $aRow['package'] . '</a>' : '');
    //$row[] = ($aRow['package']);
    ...
    if (has_permission('customers', '', 'edit')){
        $options .= icon_url('clients/client/' . $aRow['userid'], 'pencil-square-o');
    }else{
        $options = "";
    }
    if ($hasPermissionDelete) {
        $options .= icon_url('clients/delete/' . $aRow['userid'], 'remove', ' _delete', array(
        'data-toggle' => 'tooltip',
        'data-placement' => 'left',
