...
class Proposals_model extends CRM_Model
{
    ...
    {
        return $this->db->query("SELECT DISTINCT(assigned) as sale_agent FROM tblproposals_perfex WHERE assigned != 0")->result_array();
    }

    public function get_proposals_years()
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM tblproposals_perfex')->result_array();
    }
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $sort = array(), $count = false)
    {
        ...
        $this->db->from('tblproposals_perfex');
        ...
        if ($search != '') {
            if (!_startsWith($search, '#')) {
                ...
                $this->db->where('tblproposals_perfex.id IN
    ...
    public function add($data)
    {
        ...
        $exists = $this->db->get('tblproposals_perfex')->row();
        if ($exists) {
            $data['hash'] = md5(rand() . microtime());
        }
        ...
        $this->db->insert('tblproposals_perfex', $data);
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblproposals_perfex', $data);
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
    ...
    public function get($id = '', $where = array(), $for_editor = false)
    {
        ...
        $this->db->select('*,tblcurrencies.id as currencyid, tblproposals_perfex.id as id, tblcurrencies.name as currency_name');
        $this->db->from('tblproposals_perfex');
        $this->db->join('tblcurrencies', 'tblcurrencies.id = tblproposals_perfex.currency', 'left');

        if (is_numeric($id)) {
            $this->db->where('tblproposals_perfex.id', $id);
    ...
    public function update_total_tax($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tblproposals_perfex', array(
            'total_tax' => $total_tax
        ));
    }
    ...
    public function copy($id)
    {
        ...
        $fields          = $this->db->list_fields('tblproposals_perfex');
        $insert_data     = array();
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete('tblproposals_perfex');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('proposalid', $id);
            $this->db->delete('tblproposalcomments');
            // Get related tasks
    ...
    public function send_proposal_to_email($id, $template = '', $attachpdf = true, $cc = '')
    {
        ...
        if ($proposal->status == 6) {
            $this->db->where('id', $id);
            $this->db->update('tblproposals_perfex', array('status'=>4));
            $proposal = $this->get($id);
        }
