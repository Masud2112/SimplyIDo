...
class Misc_model extends CRM_Model
{
    ...
    public function get_taxes_dropdown_template($name, $taxname, $type = '', $item_id = '', $is_edit = false, $manual = false)
    {
        ...
        foreach ($taxes as $tax) {
            unset($taxes[$i]['id']);
            $taxes[$i]['taxname'] = $tax['name'] . '|' . $tax['taxrate'];
            $i++;
        }
        ...
        if ($is_edit == true) {
            ...
            foreach ($item_taxes as $item_tax) {
                if (!in_array($item_tax['taxname'], array_column($taxes, 'taxname'))) {
                    $new_tax = array();
                    $new_tax['name'] = $item_tax['name'];
                    $new_tax['taxrate'] = $item_tax['taxrate'];
                    $new_tax['taxname'] = $item_tax['taxname'];
                    $taxes[] = $new_tax;
                }
            }
        }
        ...
        if (is_array($taxname)) {
            foreach ($taxname as $tax) {
                ...
                //if (!value_exists_in_array_by_key($taxes, 'taxname', $tax)) {
                if (!in_array($tax['taxname'], array_column($taxes, 'taxname'))) {
                    if (!is_array($tax)) {
                        $tmp_taxname = $tax;
                    ...
                    $taxes[] = array('taxname' => $tmp_taxname, 'name' => $tax_array[0], 'taxrate' => $tax_array[1]);
                }
            }
        }
        ...
        foreach ($taxes as $tax) {
            ...
            if (is_array($taxname)) {
                ...
                if ($taxname == $tax['taxname']) {
                    $selected = 'selected';
                }
            }

            $select .= '<option value="' . $tax['taxname'] . '" ' . $selected . ' data-taxrate="' . $tax['taxrate'] . '" data-taxname="' . $tax['name'] . '" data-subtext="' . $tax['name'] . '" onchange="calculate_total();" >' . $tax['taxrate'] . '%</option>';
        }
        $select .= '</select>';
    ...
    public function add_attachment_to_database($rel_id, $rel_type, $attachment, $external = false)
    {
         ...
         * Modified By : Vaidehi
         * Dt : 11/21/2017
         * to not unset staff id for attachment when copying task
         */
        if (isset($attachment[0]['contact_id']) && $attachment[0]['contact_id'] != 0) {
     ...
     * Added By : Vaidehi
     * Dt : 11/16/2017
     * to get application time zone offset
     */
    public function get_timezoneoffset()
    {
        $default_timezone = get_option('default_timezone');

        $query = $this->db->query("SELECT `timezoneoffset` FROM `tbltimezoneutcoffset` WHERE `timezonename` = '" . $default_timezone . "'");
    ...
    public function get_notes($rel_id, $rel_type)
    {
         ...
         * Added By : Vaidehi
         * Dt : 11/16/2017
         * to sort notes for leads and meetings
         */
        if ($rel_type == 'leads') {
            $query = $this->db->query("SELECT n.*, s.*, l.`eventtimezone` FROM `tblnotes` n JOIN `tblstaff` s ON n.addedfrom = s.staffid JOIN `tblleads` l ON n.`rel_id` = l.`id` WHERE n.`rel_id` = " . intval($rel_id) . " AND n.`rel_type` = '" . $rel_type . "' ORDER BY (SELECT CONVERT_TZ(n.`dateadded`, (SELECT `timezoneoffset` FROM `tbltimezoneutcoffset` WHERE `timezonename` = (SELECT `value` FROM `tbloptions` WHERE `name` = 'default_timezone')), (SELECT `timezoneoffset` FROM `tbltimezoneutcoffset` WHERE `timezonename` = l.eventtimezone))) DESC");
            return $query->result_array();
        } else if ($rel_type == 'meeting') {
            $query = $this->db->query("SELECT n.*, s.*, m.`default_timezone` FROM `tblnotes` n JOIN `tblstaff` s ON n.addedfrom = s.staffid JOIN `tblmeetings` m ON n.`rel_id` = m.`meetingid` WHERE n.`rel_id` = " . intval($rel_id) . " AND n.`rel_type` = '" . $rel_type . "' ORDER BY (SELECT CONVERT_TZ(n.`dateadded`, (SELECT `timezoneoffset` FROM `tbltimezoneutcoffset` WHERE `timezonename` = (SELECT `value` FROM `tbloptions` WHERE `name` = 'default_timezone')), (SELECT `timezoneoffset` FROM `tbltimezoneutcoffset` WHERE `timezonename` = m.default_timezone))) DESC");
            return $query->result_array();
        } else if ($rel_type == 'project') {
            $this->db->select('id');
            $this->db->where('(parent = ' . $rel_id . ' OR id = ' . $rel_id . ')');
    ...
    public function edit_note($data, $id)
    {
         ...
         * Modified By : Vaidehi
         * Dt : 11/22/2017
         * added updatedfrom and dateupdated clause in table
         */
        $this->db->update('tblnotes', array(
            'description' => nl2br($data['description']),
            'updatedfrom' => get_staff_user_id(),
    ...
    public function delete_note($note_id, $leadid = "")
    {
        $this->db->where('id', $note_id);
        $note = $this->db->get('tblnotes')->row();
        if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
            //return _l('problem_deleting', _l('notes'));
            return false;
        }
    ...
    public function delete_project_note($note_id, $projectid = "")
    {
        $this->db->where('id', $note_id);
        $note = $this->db->get('tblnotes')->row();
        if ($note->addedfrom != get_staff_user_id() && !is_admin()) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $projectid = $projectid;
            $message = 'Deleted note';
            $this->projects_model->log_project_activity($note_id, $message);
    ...
    public function get_user_notifications($read = 1)
    {
        ...
        //$this->db->where('description != ', 'not_lead_added_attachment');
        //$this->db->group_by("title");
        $this->db->limit($total);
        $this->db->order_by('date', 'desc');
        /*echo "<pre>";
        print_r($this->db->get('tblnotifications')->result_array());
    ...
    public function perform_search($q)
    {
        ...
        // Payments search
        $has_permission_view_payments = has_permission('account_setup', '', 'view');
        $has_permission_view_invoices_own = has_permission('invoices', '', 'view_own');

        if (has_permission('account_setup', '', 'view') || $has_permission_view_invoices_own) {
            if (is_numeric($q)) {
                $q = trim($q);
                $q = ltrim($q, '0');
    ...
    public function replace_changed_lang_keys_v162()
    {
        $changed_strings = array(
            ...
            'defaut_projects_kanban_sort' => 'default_projects_kanban_sort',
            'defaut_projects_kanban_sort_type' => 'default_projects_kanban_sort_type',
    ...
    function get_social($id)
    {
        ...
        return $this->db->get('tblsocials')->row()->name;
    }

    function addressbook_email_exists($data)
    {
        ...
        if ($ispublic == 1) {
            ...
            $get_brand = array_map('current', $get_brand);
            $get_brand = implode(', ', $get_brand);
        }

        if ($addressbookid != '') {
            $this->db->select('tbladdressbookemail.email');
            $this->db->join('tbladdressbook', 'tbladdressbook.addressbookid=tbladdressbookemail.addressbookid', 'inner');
            $this->db->join('tbladdressbook_client', 'tbladdressbook.addressbookid=tbladdressbook_client.addressbookid', 'inner');
            $this->db->where('tbladdressbookemail.email', $data['email']);
            if ($ispublic == 1) {
                $this->db->where('tbladdressbook_client.brandid IN(' . $get_brand . ')');
                $this->db->where('(tbladdressbook.ispublic=1 OR tbladdressbook.brandid IN (' . $get_brand . '))');
            } else {
            ...
            if ($addressbookemailid != "" && $addressbookemailid > 0) {
                $this->db->where('tbladdressbookemail.addressbookemailid !=', $addressbookemailid);
            }
            $total_rows = $this->db->count_all_results('tbladdressbookemail');
        } else {
            $this->db->select('tbladdressbookemail.email');
            $this->db->join('tbladdressbook', 'tbladdressbook.addressbookid=tbladdressbookemail.addressbookid', 'inner');
            $this->db->join('tbladdressbook_client', 'tbladdressbook.addressbookid=tbladdressbook_client.addressbookid', 'inner');
