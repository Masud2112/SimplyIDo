...
class Cron_model extends CRM_Model
{
    ...
    public function run($manually = false)
    {
        ...
        $this->recurring_invoices_autopay();
    ...
    private function tasks_reminders()
    {
        ...
        $currentdate = (string)date('Y-m-d H:i');
        $notifiedUsers = array();
        $currenttime = strtotime($currentdate);
        foreach ($tasks as $task) {
            $this->db->where('taskid', $task['id']);
            $reminders = $this->db->get('tbltaskreminders')->result_array();
            $duedate = $task['duedate'];
            foreach ($reminders as $reminder) {
                if ($reminder['interval'] == "hours") {
                    $remindertime = strtotime('-' . $reminder['duration'] . ' hours', strtotime($duedate)) . "<br />";
                } elseif ($reminder['interval'] == "minutes") {
                    $remindertime = strtotime('-' . $reminder['duration'] . ' minutes', strtotime($duedate));

                } else {
                    $remindertime = strtotime('-' . $reminder['duration'] . ' days', strtotime($duedate));
                }
    ...
    public function getcronmeetings()
    {
        ...
        foreach ($meetings as $meeting) {
            $duedate = $meeting['start_date'];
            if (strtotime($duedate) >= $currenttime) {
                $this->db->where('meetingid', $meeting['meetingid']);
                $reminders = $this->db->get('tblmeetingreminders')->result_array();
                foreach ($reminders as $reminder) {
                    if ($reminder['meetinginterval'] == "hours") {
                        $remindertime = strtotime('-' . $reminder['duration'] . ' hours', strtotime($duedate)) . "<br />";
                    } elseif ($reminder['meetinginterval'] == "minutes") {
                        $remindertime = strtotime('-' . $reminder['duration'] . ' minutes', strtotime($duedate));
                    ...
                    if ($remindertime == $currenttime) {
                        if ($meeting['location'] > 0) {
                            $meeting['location'] = get_meeting_location($meeting['location']);
                        }
                        $merge_fields = array();
                        $merge_fields = array_merge($merge_fields, get_meeting_reminder_merge_field($meeting));
                        if (empty($meeting['email'])) {
                            $meeting['email'] = $meeting['contactemail'];
                        }
    ...
    private function recurring_invoices_autopay()
    {
        ...
        foreach ($invoices as $inv) {
            ...
            $this->db->where('invoiceid', $invoice->is_recurring_from);
            $custid = $this->db->get('tblinvoicepaymentrecords')->row()->custid;

            $data = array();
            $data['invoiceid'] = $invoice->id;
            $data['amount'] = $invoice->total;
            $data['custid'] = (string)$custid;
