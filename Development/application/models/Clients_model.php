...
class Clients_model extends CRM_Model
{
    ...
    public function get($id = '', $where = array('tblclients.active' => 1))
    {
       //$this->db->select(implode(',', prefixed_table_fields_array('tblclients')) . ',CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE company END as company');
        ...
        $this->db->join('tblstaff', 'tblstaff.staffid  = tblclients.primary_user_id', 'inner');
    ...
    public function update($data, $id, $client_request = false)
    {
        ...
        $this->db->select('primary_user_id');
        $this->db->where('userid', $id);
        $clientdata = $this->db->get('tblclients')->row();
        $primary_user_id = $clientdata->primary_user_id;
        
        $this->db->where('staffid', $primary_user_id);
    ...
    public function delete($id)
    {
        ...
        //Added by Vaidehi on 04/12/2018 to get everything related to client
        $users = $this->db->query('SELECT `userid`, `brandid` FROM `tblbrand` WHERE `userid` IN (' . $id . ') GROUP BY `userid`')->result_array();
        ...
        // $this->db->delete('tblclients');
         if ($this->db->affected_rows() > 0) {
            ...
            //delete all leads
            $lead_data['deleted']      = 1;
            $lead_data['updatedby']    = $this->session->userdata['staff_user_id'];
            $lead_data['dateupdated']  = date('Y-m-d H:i:s');
            
            $this->db->where('brandid IN ('. $brandid .')');
            $this->db->update('tblleads', $lead_data);

            //delete all projects
            $project_data['deleted']      = 1;
            $project_data['updatedby']    = $this->session->userdata['staff_user_id'];
            $project_data['dateupdated']  = date('Y-m-d H:i:s');
            
            $this->db->where('brandid IN ('. $brandid .')');
            $this->db->update('tblprojects', $project_data);

            //delete all tasks
            $task_data['deleted']      = 1;
            $task_data['updated_by']   = $this->session->userdata['staff_user_id'];
            $task_data['dateupdated']  = date('Y-m-d H:i:s');
            
            $this->db->where('brandid IN ('. $brandid .')');
            $this->db->update('tblstafftasks', $task_data);

            //delete all meetings
            $meeting_data['deleted']        = 1;
            $meeting_data['updated_by']     = $this->session->userdata['staff_user_id'];
            $meeting_data['updated_date']   = date('Y-m-d H:i:s');
            
            $this->db->where('brandid IN ('. $brandid .')');
            $this->db->update('tblmeetings', $meeting_data);

            //get all staffid for all brand associated with client
            $staffs = $this->db->query('SELECT `staffid` FROM `tblstaffbrand` WHERE `brandid` IN (' . $brandid . ')')->result_array();
            ...
            $staffid    = rtrim($staffid, ",");

            //delete all staffbrand
            $staff_brand_data['active']      = 0;
            
            $this->db->where('brandid IN ('. $brandid .')');
            $this->db->update('tblstaffbrand', $staff_brand_data);

            //delete all staff
            $staff_data['deleted']       = 1;
            $staff_data['updated_by']    = $this->session->userdata['staff_user_id'];
            $staff_data['updated_date']  = date('Y-m-d H:i:s');
            
            $this->db->where('staffid IN ('. $staffid .')');
            $this->db->update('tblstaff', $staff_data);

            //delete all brands
            $brand_data['deleted']      = 1;
            $brand_data['updated_by']   = $this->session->userdata['staff_user_id'];
            $brand_data['dateupdated']  = date('Y-m-d H:i:s');
