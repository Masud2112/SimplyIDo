...
class Authentication extends CI_Controller
{
    ...
    public function admin()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                } elseif (is_array($data) && isset($data['trialexpire'])) {
                    set_alert('danger', _l('admin_auth_expire_invalid'));
                    redirect(site_url('authentication/admin'));
    ...
    public function sidoadmin()
    {
        ...
        if ($this->input->post()) {
            if ($this->form_validation->run() !== false) {
                ...
                if (isset($remember) && $remember == 1) {
                    ...
                    setcookie ('cook_sidoadmin_username', $email, time() + 100000);
                    setcookie ('cook_sidoadmin_password', $password,time() + 100000);
                    setcookie ('Sidoadmin_RememberMe', "1",time() + 100000);
                                            
                }else{
                    setcookie ('cook_sidoadmin_username', '', time()-100000);
                    setcookie ('cook_sidoadmin_password', '', time()-100000);
                    setcookie ('Sidoadmin_RememberMe', '', time()-100000);
                }
                if (is_array($data) && isset($data['memberinactive'])) {
                    set_alert('danger', _l('admin_auth_inactive_account'));
                    redirect(site_url('authentication/admin'));
        ...
        $data['title'] = _l('admin_auth_login_heading');
        $this->load->view('authentication/login_sidoadmin', $data);
    }
    ...
    public function logout()
    {
        ...
        if($is_sido_admin == 1){
            redirect(site_url('authentication/sidoadmin'));
        }else{
            redirect(site_url('authentication/admin'));
        }
    }
