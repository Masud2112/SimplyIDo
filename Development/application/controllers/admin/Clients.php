...
class Clients extends Admin_controller
{
    ...
    {
        if (!has_permission('customers', '', 'view', true)) {
            if (!have_assigned_customers() && !has_permission('customers', '', 'create', true)) {
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                if (!has_permission('customers', '', 'create', true)) {
                    access_denied('customers');
                }
                ...
                if (!has_permission('customers', '', 'edit', true)) {
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                if(!has_permission('invoices','','view', true) && !has_permission('account_setup','','view', true)){
    ...
    {
        if (!has_permission('customers', '', 'create', true) && !has_permission('customers', '', 'edit', true)) {
            access_denied('customers');
        }
    ...
    {
        if (!has_permission('customers', '', 'delete', true)) {
            if (!is_customer_admin($customer_id)) {
                access_denied('customers');
            }
        }
    ...
    {
        if (has_permission('customers', '', 'delete', true) || is_customer_admin($customer_id)) {
    ...
    {
        if (has_permission('customers', '', 'edit', true) || is_customer_admin(get_user_id_by_contact_id($id))) {
            if ($this->input->is_ajax_request()) {
    ...
    {
        $has_permission_view = has_permission('invoices', '', 'view', true);
        if (!$has_permission_view && !has_permission('invoices', '', 'view_own', true)) {
    ...
    {
        $has_permission_view = has_permission('estimates', '', 'view', true);
        if (!$has_permission_view && !has_permission('estimates', '', 'view_own', true)) {
    ...
    public function zip_payments($id)
    {
        ...
        $has_permission_view = has_permission('account_setup', '', 'view', true);
        if (!$has_permission_view && !has_permission('invoices', '', 'view_own', true)) {
