...
class Contracts extends Admin_controller
{
    ...
    {
        if (!has_permission('contracts', '', 'view', true) && !has_permission('contracts', '', 'view_own', true)) {
    ...
    public function contract($id = '')
    {
        if ($this->input->post()) {
            if ($id == '') {
                if (!has_permission('contracts', '', 'create', true)) {
                    access_denied('contracts');
                }
                ...
                if (!has_permission('contracts', '', 'edit', true)) {
                    access_denied('contracts');
                }
        ...
        if ($id == '') {
            ...
            if (!$data['contract'] || (!has_permission('contracts', '', 'view', true) && $data['contract']->addedfrom != get_staff_user_id())) {
    ...
    {
        if (!has_permission('contracts', '', 'edit', true) && !has_permission('contracts', '', 'create', true)) {
    ...
    {
        if (!has_permission('contracts', '', 'create', true) && !has_permission('contracts', '', 'edit', true)) {
            access_denied('contracts');
        }
        if ($this->input->post()) {
    ...
    {
        if (!has_permission('contracts', '', 'delete', true)) {
            access_denied('contracts');
        }
        if (!$id) {
            redirect(admin_url('contracts'));
