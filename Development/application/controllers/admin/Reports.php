...
class Reports extends Admin_controller
{
    ...
    {
        parent::__construct();
        if (!has_permission('reports', '', 'view', true)) {
    ...
    */
    ...
    {
        $is_sido_admin = is_sido_admin(); 
        $data['title']           = _l('reports');

        if($is_sido_admin) {
            $report_data            = $this->reports_model->get_configuration();
            $data['report_data']    =  $report_data;
            $this->load->view('admin/reports/admin_reports', $data);
        } else {
            $report_data            = $this->reports_model->get_configuration();
            $data['report_data']    =  $report_data;
    ...
    * Added By: Vaidehi
    * Dt: 03/08/2018
    * for expanded lead source reports
    */
    public function leadsource()
    {
        $report_data        = $this->reports_model->get_configuration('Lead Source'); 
        $report_arr = (array) $report_data;
        $data['filter_val']                 = $report_arr['saved_filter'];
        $data['reportconfigurationid']      = $report_arr['reportconfigurationid'];
        $data['lead_sources']               = $this->reports_model->getleadsources_report($report_arr);
        $data['report_data']                = $report_arr;
        $this->load->view('admin/reports/leadsource', $data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/08/2018
    * for expanded lead status reports
    */
    public function leadstatus()
    {
        $report_data        = $this->reports_model->get_configuration('Booking Success'); 
        $report_arr = (array) $report_data;
        $data['filter_val']                 = $report_arr['saved_filter'];
        $data['reportconfigurationid']      = $report_arr['reportconfigurationid'];
        $data['lead_statuses']              = $this->reports_model->getleadstatus_report($report_arr);
        $data['report_data']                = $report_arr;
        $this->load->view('admin/reports/leadstatus', $data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/08/2018
    * for expanded revenue reports
    */
    public function revenue()
    {
        $report_data        = $this->reports_model->get_configuration('Revenue'); 
        $report_arr = (array) $report_data;
        $data['filter_val']                 = $report_arr['saved_filter'];
        $data['reportconfigurationid']      = $report_arr['reportconfigurationid'];
        $data['revenues']                   = $this->reports_model->getrevenues_report($report_arr);
        $data['report_data']                = $report_arr;
    ...
    */
    ...
    {   
        $data['title']           = _l('report_configuration');

        $data['report_data']     = $this->reports_model->get_configuration();

        $this->load->view('admin/reports/configuration',$data);    
    }
    ...
    * for order update
    */
    public function ajax_order_update()
    {   
        $data = json_decode($this->input->post('options'));
        foreach ($data as $record) {
            $config_data                        = (array) $record;
            $success = $this->reports_model->update_configuration($config_data);
        }
        die('<--here');
    }
    ...
    * Added By: Vaidehi
    * Dt: 03/13/2018
    * to update show/hide on reports
    */
    ...
    { 
        ...
        if(isset($type)) {
            if($success) {
                set_alert('success', _l('configuration_saved_successfully'));
                redirect(admin_url('reports'));
            } else {
                set_alert('danger', _l('problem_updating_configuration'));
                redirect(admin_url('reports'));
            }
        } else {
            echo $success;
    ...
    */
    public function filter_revenue() {
        ...
        $type               = $this->input->post('type');
        $default_startdate  = $this->input->post('startDate');
        $default_enddate    = $this->input->post('endDate');

        $report_data        = $this->reports_model->get_configuration('Revenue'); 
        $report_arr         = (array) $report_data;

        $data['filters']                                = $this->reports_model->get_filters($report_arr['reportconfigurationid']);
        $data['roles']                                  = $this->roles_model->get();

        if(!empty($type)) {
           $data['detailed_report']         = true;
        } else {
            $data['detailed_report']        = false;
        }

        $search_data                                = [];
        $search_data['saved_filter']                = $this->input->post('saved_filter');

        if(!empty($default_startdate)) {
            $search_data['startDate']               = $default_startdate;
        } elseif(!empty($report_arr['start_date'])) {
            $search_data['start_date']              = $report_arr['start_date'];
        }

        if(!empty($default_enddate)) {
            $search_data['endDate']                 = $default_enddate;
        } elseif(!empty($report_arr['end_date'])) {
            $search_data['end_date']                = $report_arr['end_date'];
        }

        $search_data['reportconfigurationid']       = $this->input->post('reportconfigurationid');

        $data['reportconfigurationid']              = $this->input->post('reportconfigurationid');
        $data['revenues']                           = $this->reports_model->getrevenues_report($search_data);

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $revenue_roles          = $this->reports_model->get_permission('Revenue');
            
            if(!empty($revenue_roles->sharing_permission)) {
                if (strpos($revenue_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_revenue']   = 1;
                } else {
                    $data['has_permission_revenue']   = 0;
                }
            } else {
                $data['has_permission_revenue']   = 0;
            }
        ...
        $this->load->view('admin/reports/revenuereport',$data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/15/2018
    * to filter lead source
    */
    public function filter_leadsource() {
        $session_data = get_session_data();
        ...
        $data['reportconfigurationid']  = $this->input->post('reportconfigurationid');
        $data['lead_sources']           = $this->reports_model->getleadsources_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $lead_source_roles      = $this->reports_model->get_permission('Lead Source');
            
            if(!empty($lead_source_roles->sharing_permission)){
                if (strpos($lead_source_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_lead_source']   = 1;
                } else {
                    $data['has_permission_lead_source']   = 0;
                }
            } else {
                $data['has_permission_lead_source']   = 0;
            }
        ...
        $this->load->view('admin/reports/leadsourcereport',$data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/15/2018
    * to filter lead status
    */
    public function filter_leadstatus() {
        $session_data = get_session_data();
        ...
        $data['reportconfigurationid']  = $this->input->post('reportconfigurationid');
        $data['lead_statuses']          = $this->reports_model->getleadstatus_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $booking_stage_roles    = $this->reports_model->get_permission('Booking Success');
            
            if(!empty($booking_stage_roles->sharing_permission)) {
                if (strpos($booking_stage_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_booking_stage']   = 1;
                } else {
                    $data['has_permission_booking_stage']   = 0;
                }
            } else {
                $data['has_permission_booking_stage']   = 0;
            }
        ...
        $this->load->view('admin/reports/leadstatusreport',$data);
    }
    ...
    */
    public function filter_signup() {
        ...
        $report_data        = $this->reports_model->get_configuration('Sign Up'); 
        $report_arr         = (array) $report_data;
        ...
        $data['reportconfigurationid']              = $this->input->post('reportconfigurationid');
        $data['signups']                            = $this->reports_model->getsignup_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $signup_roles           = $this->reports_model->get_permission('Sign Up');
           
            if(!empty($signup_roles->sharing_permission)){
                if (strpos($signup_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_signup']   = 1;
                } else {
                    $data['has_permission_signup']   = 0;
                }
            } else {
                $data['has_permission_signup']   = 0;
            }
        ...
        $this->load->view('admin/reports/signupreport',$data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/22/2018
    * to filter subscriber
    */
    public function filter_subscriber() {
        $session_data = get_session_data();
        ...
        $report_data        = $this->reports_model->get_configuration('Subscribers'); 
        $report_arr         = (array) $report_data;
        ...
        $data['reportconfigurationid']  = $this->input->post('reportconfigurationid');
        $data['subscribers']            = $this->reports_model->getsubscriber_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $subscriber_roles       = $this->reports_model->get_permission('Subscriber');
           
            if(!empty($subscriber_roles->sharing_permission)){
                if (strpos($subscriber_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_subscriber']   = 1;
                } else {
                    $data['has_permission_subscriber']   = 0;
                }
            } else {
                $data['has_permission_subscriber']   = 0;
            }
        ...
        $this->load->view('admin/reports/subscriberreport',$data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 03/22/2018
    * to filter conversion rate
    */
    public function filter_conversionrate() {
        $session_data = get_session_data();
        ...
        $report_data        = $this->reports_model->get_configuration('Conversion Rate'); 
        $report_arr         = (array) $report_data;
        ...
        $data['reportconfigurationid']              = $this->input->post('reportconfigurationid');
        $data['conversionrates']                    = $this->reports_model->getconversionrate_report($search_data);

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $conversion_rate_roles       = $this->reports_model->get_permission('Conversion Rate');
           
            if(!empty($conversion_rate_roles->sharing_permission)){
                if (strpos($conversion_rate_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_conversion_rate']   = 1;
                } else {
                    $data['has_permission_conversion_rate']   = 0;
                }
            } else {
                $data['has_permission_conversion_rate']   = 0;
            }
        ...
        $this->load->view('admin/reports/conversionratereport',$data);
    }
    ...
    */
    public function filter_churn() {
        ...
        $report_data        = $this->reports_model->get_configuration('Churn'); 
        $report_arr         = (array) $report_data;
        ...
        $data['reportconfigurationid']              = $this->input->post('reportconfigurationid');
        $data['churns']                             = $this->reports_model->getchurn_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $signup_roles           = $this->reports_model->get_permission('Churn');
           
            if(!empty($signup_roles->sharing_permission)){
                if (strpos($signup_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_churn']   = 1;
                } else {
                    $data['has_permission_churn']   = 0;
                }
            } else {
                $data['has_permission_churn']   = 0;
            }
        ...
        $this->load->view('admin/reports/churnreport',$data);
    }

    /**
    * Added By: Vaidehi
    * Dt: 04/06/2018
    * to filter net revenue
    */
    public function filter_netrevenue() {
        $session_data = get_session_data();
        ...
        $report_data        = $this->reports_model->get_configuration('Net Revenue'); 
        $report_arr         = (array) $report_data;

        $data['filters']                                = $this->reports_model->get_netrevenuefilters($report_arr['reportconfigurationid']);
        $data['roles']                                  = $this->roles_model->get();
        ...
        $data['reportconfigurationid']              = $this->input->post('reportconfigurationid');
        $data['netrevenues']                        = $this->reports_model->getnetrevenue_report($search_data); 

        //check for permission
        if($session_data['user_type'] == 2) {
            $staff_roles            = $this->staff_model->get_staff_role();            
            $signup_roles           = $this->reports_model->get_permission('Net Revenue');
           
            if(!empty($signup_roles->sharing_permission)){
                if (strpos($signup_roles->sharing_permission, $staff_roles->roleid)  >= 0) {
                    $data['has_permission_netrevenue']   = 1;
                } else {
                    $data['has_permission_netrevenue']   = 0;
                }
            } else {
                $data['has_permission_netrevenue']   = 0;
            }
        ...
        $this->load->view('admin/reports/netrevenuereport',$data);
    }
