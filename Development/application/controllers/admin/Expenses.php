...
class Expenses extends Admin_controller
{
    ...
    {
        if (!has_permission('expenses', '', 'view', true) && !has_permission('expenses', '', 'view_own', true)) {
    ...
    public function expense($id = '')
    {
        if ($this->input->post()) {
            if ($id == '') {
                if (!has_permission('expenses', '', 'create', true)) {
                ...
                if (!has_permission('expenses', '', 'edit', true)) {
                    set_alert('danger', _l('access_denied'));
                    echo json_encode(array(
        ...
        if ($id == '') {
            ...
            if (!$data['expense'] || (!has_permission('expenses', '', 'view', true) && $data['expense']->addedfrom != get_staff_user_id())) {
    ...
    {
        if (!has_permission('expenses', '', 'delete', true)) {
            access_denied('expenses');
        }
        if (!$id) {
    ...
    public function get_expense_data_ajax($id)
    {
        ...
        if (!$expense || (!has_permission('expenses', '', 'view', true) && $expense->addedfrom != get_staff_user_id())) {
