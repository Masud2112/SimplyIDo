...
class Misc extends Admin_controller
{
    ...
    public function delete_reminder($rel_id, $id, $rel_type)
    {
        ...
        // }
        if ($success) {
            set_alert('success', _l('reminder_deleted'));
        } else {
            set_alert('danger', _l('reminder_failed_to_delete'));
        }
    }
    ...
    /* Check if agreement title exist */
    public function agreement_title_exists()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post()) {
                ...
                if ($agreementid != '') {
                    ...
                    $this->db->where('deleted', 0);
                    $this->db->where('templateid', $agreementid);
                    $_current_name = $this->db->get('tblagreementtemplates')->row();
                    if ($_current_name->name == $this->input->post('name')) {
                        echo json_encode(true);
                        die();
                    }
                }
                ...
                $this->db->where('brandid', $brandid);
                $this->db->where('deleted', 0);
                $total_rows = $this->db->count_all_results('tblagreementtemplates');
                if ($total_rows > 0) {
                    echo json_encode(false);
                } else {
                    echo json_encode(true);
    ...
    /* Check if addressbook email exists / ajax */
    public function addressbook_email_exists()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                $total_rows = $this->misc_model->addressbook_email_exists($this->input->post());
                if ($total_rows > 0) {
                    echo json_encode(false);
                } else {
                    echo json_encode(true);
    ...
    /* Check if addressbook email exists / ajax */
    public function venue_email_exists()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                // First we need to check if the email is the same
                ...
                if ($venueid != '') {
                    $this->db->where('addressbookid !=', $venueid);
                    $this->db->where('email', $this->input->post('email'));
                    $total_rows = $this->db->count_all_results('tblvenueemail');
                    if ($total_rows > 0) {
                        echo json_encode(false);
                    } else {
                        echo json_encode(true);
    ...
    /* Check if agreement title exist */
    public function payment_title_exists()
    {
        if ($this->input->is_ajax_request()) {
            $brandid = get_user_session();
            if ($this->input->post()) {
                // First we need to check if the email is the same
                $paymentscheduleid = $this->input->post('paymentscheduleid');
                if ($paymentscheduleid != '') {
                    $this->db->where('brandid', $brandid);
                    $this->db->where('deleted', 0);
                    $this->db->where('templateid', $paymentscheduleid);
                    $_current_name = $this->db->get('tblpaymenttemplates')->row();
                    if ($_current_name->name == $this->input->post('name')) {
                        echo json_encode(true);
                        die();
                    }
                ...
                $this->db->where('brandid', $brandid);
                $this->db->where('deleted', 0);
                $total_rows = $this->db->count_all_results('tblpaymenttemplates');
                if ($total_rows > 0) {
                    echo json_encode(false);
                } else {
                    echo json_encode(true);
    ...
    {
        $result = $this->misc_model->get_social($id);
        $result = strtolower($result);

        if ($result == "facebook") {
            $url = "http://facebook.com";
        } elseif ($result == "twitter") {
            $url = "http://facebook.com";
        } elseif ($result == "instagram") {
            $url = "http://instagram.com";
        } elseif ($result == "blog") {
            $url = "http://blogger.com";
        } elseif ($result == "youtube") {
            $url = "http://youtube.com";
        } elseif ($result == "pintrest") {
            $url = "http://pintrest.com";
        } elseif ($result == "snapchat") {
            $url = "http://snapchat.com";
        } elseif ($result == "linkedin") {
    ...
    public function service_type_exists()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                // First we need to check if the email is the same
                ...
                if ($brand_type != '') {
                    $this->db->where('name', $brand_type);
                    //$brand_type_exist = $this->db->get('tblbrandtype')->row();
    ...
    {
        if ($this->input->post()) {
            $data =array('name'=>$this->input->post('brand_type'));
            $result = $this->misc_model->addservicetype($data);
            if($result > 0){
                set_alert('success', _l('added_successfully', _l('service_type')));
                redirect(admin_url('brand_settings?group=company'));
            }
        }
    }
}
