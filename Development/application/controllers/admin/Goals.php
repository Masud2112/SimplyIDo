...
class Goals extends Admin_controller
{
    ...
    {
        if (!has_permission('goals', '', 'view', true)) {
            access_denied('goals');
        }
    ...
    public function goal($id = '')
    {
        ...
        if ($this->input->post()) {
            if ($id == '') {
                if (!has_permission('goals', '', 'create', true)) {
                    access_denied('goals');
                }
                ...
                if (!has_permission('goals', '', 'edit', true)) {
                    access_denied('goals');
                }
    ...
    {
        if (!has_permission('goals', '', 'delete', true)) {
            access_denied('goals');
        }
        if (!$id) {
    ...
    {
        if (!has_permission('goals', '', 'edit', true) && !has_permission('goals', '', 'create', true)) {
            access_denied('goals');
        }
        if (!$id) {
            redirect(admin_url('goals'));
