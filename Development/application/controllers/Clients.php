...
class Clients extends Clients_controller
{
    ...
    public function viewinvoice($id = '', $hash = '')
    {
        ...
        if($proposalid > 0 ){
            ...
            $invoice->proposal = $this->proposaltemplates_model->getproposaltemplates($proposalid);
        ...
        if (is_client_logged_in()) {
            ...
            foreach ($statuses as $status) {
                ...
                array_push($chart['datasets'], array(
                    'label' => format_invoice_status($status, '', true),
    ...
    public function replymessage($id = '', $hash = '')
    {
        ...
        if($path != ""){
            ...
            $mid_data = $fullpath[0];
            $cid_data = $fullpath[1];
            $mid = explode('mid=',$mid_data);
            $cid = explode('cid=',$cid_data);
            $mid = end($mid);
            $cid = end($cid);

            $invoice = array();
            $data['contact_id'] = $cid;
            $data['messages'] = $this->messages_model->getclientmessagedetails($mid);
            $data['title']         = "Reply Message";
            $data['bodyclass']     = 'replymessage';
            $data['contacts']   = $this->messages_model->getclientcontacts($mid,$data['messages']->brandid);
            $data['teammember'] = $this->messages_model->getclientteammember($mid,$data['messages']->brandid);
            $this->use_navigation  = false;
            $this->use_submenu     = false;
            $this->data            = $data;
            $this->view            = 'replymessage';
            add_views_tracking('invoice', $id);
            do_action('invoice_html_viewed', $id);
            $this->layout();  
        }
    ...
    public function replyclientmessage($id = '')
    {
        if ($this->input->post()) {
            $data = $this->input->post();
            $msg_id = $data['message_id'];;
            $insert_id = $this->messages_model->replyclientmessage($this->input->post(),$id);
            if ($insert_id) {
                ...
                $this->sendmail($msg_id,$insert_id);
    ...
    public function sendmail($message_id = "7", $child_id = ""){
        $this->load->model('emails_model');
        $merge_fields = array();
        ...
        //echo "<pre>";print_r($message_emails);print_r($message_details);exit;
        $privacy = "";
        ...
        $message_thread .= '<a href="{reply_link}" style="display: inline-block; background: rgb(0, 169, 185) none repeat scroll 0% 0%; color: rgb(255, 255, 255); text-decoration: none; border-radius: 4px; line-height: 20pt; width: 50px; text-align: center;">Reply</a><br><br>';
        ...
        foreach ($message_emails->usersdetail as $fvalue) {
            $reply_url = "?mid=".$message_id."&cid=".$fvalue['uid'];
            ...
            // echo "<pre>";
            // print_r($merge_fields['{message_thread}']);
    ...
    * Added By : Vaidehi
    * Dt : 01/10/2018
    * view invite for invited vendor
    */
    public function viewinvite($id) {
        $this->load->model('projects_model');

        $data['invite_details'] = $this->projects_model->get_invitees($id, 'vendor-view');
        $data['title']    = 'Invite Detail';
        
        $this->use_navigation  = false;
        $this->use_submenu     = false;
        $this->data            = $data;
        $this->view            = 'viewinvite';

        $this->session->set_userdata(array('brand_id'=>$data['invite_details']->brandid));
       /* echo "<pre>";
        print_r($data['invite_details']);
        print_r($_SESSION);
        die();*/
        $this->layout(); 
    }
    ...
    */
    public function invite_status_change()
    {
        ...
        if(!empty($this->input->post('vendor_accept'))) {
            $data['vendor_accept']     = $this->input->post('vendor_accept');
        } else {
            $data['vendor_accept']     = 0;
        }

        if(!empty($this->input->post('vendor_accept')) && $this->input->post('vendor_accept')==1) {
